...
FiniteVolumeGrid2D::FiniteVolumeGrid2D()
    : interiorFaces_("InteriorFaces"), boundaryFaces_("BoundaryFaces"),
      localCells_("LocalCells"), globalCells_("GlobalCells"),
...
void FiniteVolumeGrid2D::init(const std::vector<Point2D> &nodes,
  ...
  for (int i = 0; i < cptr.size() - 1; ++i)
    createCell(
        std::vector<Label>(cind.begin() + cptr[i], cind.begin() + cptr[i + 1]));
...
void FiniteVolumeGrid2D::reset() {
  ...
  //- Communication zones
  sendCellGroups_
      .clear(); // shared pointers are used so that zones can be moveable!
  bufferCellGroups_.clear();

  //- Face related data
  faces_.clear();
  faceDirectory_
      .clear(); // A directory that can find a face given the two node ids
...
CellGroup
FiniteVolumeGrid2D::globalCellGroup(const CellGroup &localGroup) const {
...
bool FiniteVolumeGrid2D::faceExists(Label n1, Label n2) const {
  ...
  return faceDirectory_.find(n1 < n2 ? make_pair(n1, n2) : make_pair(n2, n1)) !=
         faceDirectory_.end();
}
...
Label FiniteVolumeGrid2D::findFace(Label n1, Label n2) const {
  ...
  auto it =
      faceDirectory_.find(n1 < n2 ? make_pair(n1, n2) : make_pair(n2, n1));

  if (it == faceDirectory_.end())
    throw Exception("FiniteVolumeGrid2D", "findFace",
                    "no face found between n1 = " + to_string(n1) +
                        ", n2 = " + to_string(n2) + ".");
...
FaceGroup &FiniteVolumeGrid2D::createPatch(const std::string &name,
...
FaceGroup &
FiniteVolumeGrid2D::createPatchByNodes(const std::string &name,
...
std::vector<Ref<const Node>>
FiniteVolumeGrid2D::findNearestNodes(const Point2D &pt, int nNodes) const {
...
std::vector<int> FiniteVolumeGrid2D::eptr() const {
  ...
  std::transform(cells_.begin(), cells_.end(), std::back_inserter(eptr),
                 [](const Cell &cell) { return cell.nodes().size(); });
...
std::vector<int> FiniteVolumeGrid2D::eind() const {
  ...
  for (const Cell &cell : cells_)
    std::transform(cell.nodes().begin(), cell.nodes().end(),
...
FiniteVolumeGrid2D::connectivityGraph() const {
...
FiniteVolumeGrid2D::patchToNodeMap() const {
  using namespace std;
...
void FiniteVolumeGrid2D::partition(const Input &input) {
  ...
  if (comm_->isMainProc()) // partition is performed on main proc
  {
    ...
    int status =
        METIS_PartMeshDual(&nElems, &nNodes, eptr().data(), eind().data(), NULL,
                           NULL, &nCommon, &nPartitions, NULL, NULL, &objVal,
                           cellPartition.data(), nodePartition.data());
    if (status == METIS_OK)
      comm_->printf("Sucessfully computed partitioning.\n");
    else
      throw Exception("FiniteVolumeGrid2D", "partition",
  ...
  auto addCellToThisProc = [this, &cellPartition](const Cell &cell,
                                                  Scalar r = 0.) -> bool {
    ...
    for (const Cell &kCell :
         globalCells_.itemsWithin(Circle(cell.centroid(), r)))
  ...
  auto gid = std::accumulate(nLocalCells.begin(),
                             nLocalCells.begin() + comm_->rank(), 0);

  for (const Cell &cell : cells_) {
    cellOwnership_[cell.id()] =
        cellPartition[cellLocalToGlobalIdMap[cell.id()]];
...
void FiniteVolumeGrid2D::initPatches(
...
void FiniteVolumeGrid2D::initCommBuffers(const std::vector<Label> &ownership,
                                         const std::vector<Label> &globalIds) {
  if (ownership.size() != cells_.size() || globalIds.size() != cells_.size())
    throw Exception(
        "FiniteVolumeGrid2D", "initCommBuffers",
