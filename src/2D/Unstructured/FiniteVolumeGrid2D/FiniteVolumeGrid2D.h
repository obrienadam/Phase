...
public:
    ...
    const CellGroup& localCells() const
    { return localCells_; }

    const CellGroup& globalCells() const
    { return globalCells_; }

    const std::vector<Label> &cellOwnership() const
    { return cellOwnership_; }

    const std::vector<Label> &globalIds() const
    { return globalIds_; }
    ...
    FaceGroup &createPatch(const std::string &name, const std::vector<Label> &faces);

    FaceGroup &createPatchByNodes(const std::string &name, const std::vector<Label> &nodes);
...
protected:
    ...
    void initPatches(const std::unordered_map<std::string, std::vector<Label>> &patches);

    void initCommBuffers(const std::vector<Label> &ownership, const std::vector<Label> &globalIds);
