...
StructuredRectilinearGrid::StructuredRectilinearGrid(const Input &input)
  ...
  Scalar convertToMeters =
      input.caseInput().get<Scalar>("Grid.convertToMeters", 1.);
  ...
  init(width, height, nCellsX, nCellsY, convertToMeters, xDimRefinements,
       yDimRefinements, origin);
}

StructuredRectilinearGrid::StructuredRectilinearGrid(
    const std::vector<Scalar> &xcoords, const std::vector<Scalar> &ycoords) {
  printf("Initiating grid from coordinates...\n");
  init(xcoords, ycoords);
}

void StructuredRectilinearGrid::init(
    Scalar width, Scalar height, Size nCellsX, Size nCellsY,
...
void StructuredRectilinearGrid::init(std::vector<Scalar> xcoords,
  ...
  if (xcoords.size() < 2 || ycoords.size() < 2)
    throw Exception("StructuredRectilinearGrid", "init",
...
Scalar StructuredRectilinearGrid::h() const {
  if (!isEquidistant())
    throw Exception("StructuredRectilinearGrid", "h",
                    "not an equidistant grid.");

  return width_ / nCellsX_;
}

std::vector<Scalar>
StructuredRectilinearGrid::refineDims(Scalar start, Scalar end,
...
void StructuredRectilinearGrid::initPatches() {
  ...
  //- x patches
  for (Label j = 0; j < nCellsY_; ++j) {
    xm.push_back(findFace(node(0, j).id(), node(0, j + 1).id()));
    xp.push_back(findFace(node(nCellsX_, j).id(), node(nCellsX_, j + 1).id()));
  }
  createPatch("x-", xm);
  createPatch("x+", xp);
  ...
  //- y patches
  for (Label i = 0; i < nCellsX_; ++i) {
    ym.push_back(findFace(node(i, 0).id(), node(i + 1, 0).id()));
    yp.push_back(findFace(node(i, nCellsY_).id(), node(i + 1, nCellsY_).id()));
  }
  createPatch("y-", ym);
  createPatch("y+", yp);
}
