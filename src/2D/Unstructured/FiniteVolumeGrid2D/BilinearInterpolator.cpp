...
BilinearInterpolator::BilinearInterpolator(
    const std::weak_ptr<const FiniteVolumeGrid2D> &grid, const Point2D &pt)
...
StaticMatrix<1, 4>
BilinearInterpolator::derivativeCoeffs(const Vector2D &n) const {
  return StaticMatrix<1, 4>({pt_.y * n.x + pt_.x * n.y, n.x, n.y, 0.}) * A_;
}

Scalar
BilinearInterpolator::operator()(const ScalarFiniteVolumeField &field) const {
  auto x = StaticMatrix<1, 4>({pt_.x * pt_.y, pt_.x, pt_.y, 1.});
  auto b = StaticMatrix<4, 1>(
      {field(cells_[0]), field(cells_[1]), field(cells_[2]), field(cells_[3])});

  return (x * A_ * b)(0, 0);
}

Vector2D
BilinearInterpolator::operator()(const VectorFiniteVolumeField &field) const {
  auto x = StaticMatrix<1, 4>({pt_.x * pt_.y, pt_.x, pt_.y, 1.});

  auto b = StaticMatrix<4, 2>({field(cells_[0]).x, field(cells_[0]).y,
...
Vector2D
BilinearInterpolator::grad(const ScalarFiniteVolumeField &field) const {
  auto b = StaticMatrix<4, 1>(
      {field(cells_[0]), field(cells_[1]), field(cells_[2]), field(cells_[3])});
...
Tensor2D
BilinearInterpolator::grad(const VectorFiniteVolumeField &field) const {
  auto b = StaticMatrix<4, 2>({field(cells_[0]).x, field(cells_[0]).y,
                               field(cells_[1]).x, field(cells_[1]).y,
                               field(cells_[2]).x, field(cells_[2]).y,
                               field(cells_[3]).x, field(cells_[3]).y});
