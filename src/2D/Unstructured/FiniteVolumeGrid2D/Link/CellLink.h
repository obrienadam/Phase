...
template <class T> class FiniteVolumeField;
...
public:
  ...
  const Vector2D &rc() const { return rCellVec_; }

  Scalar alpha(const Point2D &pt) const;

  Scalar linearInterpolate(const FiniteVolumeField<Scalar> &phi,
                           const Point2D &pt) const;

  Vector2D linearInterpolate(const FiniteVolumeField<Vector2D> &u,
                             const Point2D &pt) const;
