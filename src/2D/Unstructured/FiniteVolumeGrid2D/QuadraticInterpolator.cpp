...
void QuadraticInterpolator::setPoint(const Point2D &pt) {
  ...
  if (isValid_) {
    Point2D x[] = {cells_[0].get().centroid(), cells_[1].get().centroid(),
                   ...
                   cells_[4].get().centroid(), cells_[5].get().centroid()};
...
Scalar
QuadraticInterpolator::operator()(const ScalarFiniteVolumeField &field) const {
  auto x = StaticMatrix<1, 6>(
      {pt_.x * pt_.x, pt_.y * pt_.y, pt_.x * pt_.y, pt_.x, pt_.y, 1.});
...
Vector2D
QuadraticInterpolator::operator()(const VectorFiniteVolumeField &field) const {
  auto x = StaticMatrix<1, 6>(
      {pt_.x * pt_.x, pt_.y * pt_.y, pt_.x * pt_.y, pt_.x, pt_.y, 1.});
...
Vector2D
QuadraticInterpolator::grad(const ScalarFiniteVolumeField &field) const {
  auto b = StaticMatrix<6, 1>({field(cells_[0]), field(cells_[1]),
                               field(cells_[2]), field(cells_[3]),
                               field(cells_[4]), field(cells_[5])});

  auto x = StaticMatrix<2, 6>({2. * pt_.x, 0., pt_.y, 1., 0., 0., 0.,
                               2. * pt_.y, pt_.x, 0., 1., 0.}) *
           A_ * b;
...
Tensor2D
QuadraticInterpolator::grad(const VectorFiniteVolumeField &field) const {
  auto b = StaticMatrix<6, 2>({
      field(cells_[0]).x,
      field(cells_[0]).y,
      field(cells_[1]).x,
