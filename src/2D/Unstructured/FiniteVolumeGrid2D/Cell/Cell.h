...
public:
  ...
  //- Ids
  Label id() const { return localId_; }

  Label localId() const { return localId_; }
  ...
  std::vector<InteriorLink> &neighbours() { return interiorLinks_; }

  std::vector<CellLink> &diagonals() { return diagonalLinks_; }

  std::vector<BoundaryLink> &boundaries() { return boundaryLinks_; }

  const std::vector<InteriorLink> &neighbours() const { return interiorLinks_; }

  const std::vector<CellLink> &diagonals() const { return diagonalLinks_; }

  const std::vector<BoundaryLink> &boundaries() const { return boundaryLinks_; }

  const std::vector<CellLink> &cellLinks() const { return cellLinks_; }
...
private:
  Label localId_,
      globalId_; // Indices for identification. Should not normally be changed
