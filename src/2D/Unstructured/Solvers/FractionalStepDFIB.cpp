...
FractionalStepDFIB::FractionalStepDFIB(
    const Input &input, const std::shared_ptr<const FiniteVolumeGrid2D> &grid)
    : FractionalStep(input, grid), fb_(*addField<Vector2D>("fb", fluid_)),
      fbEqn_(input, fb_, "fbEqn"), extEqn_(input, gradP_, "extEqn"),
      ib_(std::make_shared<DirectForcingImmersedBoundary>(input, grid,
                                                          fluid_)) {
  ib_->updateCells();
...
Scalar FractionalStepDFIB::solveUEqn(Scalar timeStep) {
  ...
  uEqn_ == fv::laplacian(mu_ / rho_, u_, 0.5) -
               fv::laplacian(mu_ / rho_, u_, 0.) + src::src(fb_);
  uEqn_.solve();
