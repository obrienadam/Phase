...
FractionalStepAxisymmetricDFIBMultiphase::
    ...
    : FractionalStepAxisymmetricDFIB(input, grid),
      ...
      gradGamma_(static_cast<ScalarGradient &>(*addField<Vector2D>(
          std::make_shared<ScalarGradient>(gamma_, fluid_)))),
      gradRho_(static_cast<ScalarGradient &>(
          *addField<Vector2D>(std::make_shared<ScalarGradient>(rho_, fluid_)))),
      fst_(input, grid, fluid_, ib_), gammaEqn_(input, gamma_, "gammaEqn") {
  rho1_ =
...
Scalar FractionalStepAxisymmetricDFIBMultiphase::solve(Scalar timeStep) {
  ...
  grid_->comm().printf(
      "Solving momentum and pressure equation and correting velocity...\n");
  solveUEqn(timeStep);
  solvePEqn(timeStep);
...
Scalar
FractionalStepAxisymmetricDFIBMultiphase::solveGammaEqn(Scalar timeStep) {
  auto beta =
      axi::cicsam::faceInterpolationWeights(u_, gamma_, gradGamma_, timeStep);

  gamma_.savePreviousTimeStep(timeStep, 1.);
  gammaEqn_ =
      (axi::ddt(gamma_, timeStep) + axi::cicsam::div(u_, gamma_, beta, 0.) ==
       0.);
  gammaEqn_.solve();
  gamma_.sendMessages();
  ...
  gammaEqn_ == axi::cicsam::div(u_, gamma_, beta, 0.5) -
                   axi::cicsam::div(u_, gamma_, beta, 0.) +
                   axi::src::src(gammaSrc_);
...
void FractionalStepAxisymmetricDFIBMultiphase::updateProperties(
    Scalar timeStep) {
  //- Update rho and mu
  ...
  mu_.computeCells([this](const Cell &c) {
    return rho_(c) / (rho1_ / mu1_ +
                      clamp(gamma_(c), 0., 1.) * (rho2_ / mu2_ - rho1_ / mu1_));
  });

  mu_.computeFaces([this](const Face &f) {
    return rho_(f) / (rho1_ / mu1_ +
                      clamp(gamma_(f), 0., 1.) * (rho2_ / mu2_ - rho1_ / mu1_));
  });
  ...
  for (const Cell &c : ib_->solidCells())
    for (const auto &nb : c.neighbours())
      if (ib_->ibObj(nb.cell().centroid()))
        (*fst_.fst())(nb.face()) = Vector2D(0., 0.);
...
Scalar FractionalStepAxisymmetricDFIBMultiphase::solveUEqn(Scalar timeStep) {
  ...
  fst_.fst()->oldField(0).faceToCellAxisymmetric(rho_, rho_.oldField(0),
                                                 *fluid_);
  sg_.sendMessages();
  fst_.fst()->sendMessages();
  gradP_.sendMessages();
  ...
  uEqn_ = (rho_ * axi::ddt(u_, timeStep) + rho_ * axi::dive(u_, u_, 0.5) ==
           axi::laplacian(mu_, u_, 0.5) +
               axi::src::src(sg_.oldField(0) + fst.oldField(0) - gradP_));
  ...
  for (const Cell &c : *fluid_)
    u_(c) += timeStep *
             (fib_(c) + gradP_(c) - sg_.oldField(0)(c) - fst.oldField(0)(c)) /
             rho_(c);
  ...
  for (const Face &f : grid_->interiorFaces()) {
    ...
    u_(f) =
        g * u_(l) + (1. - g) * u_(r) + timeStep / rho_(f) * (fst(f) + sg_(f));
  }
...
void FractionalStepAxisymmetricDFIBMultiphase::correctVelocity(
    Scalar timeStep) {
...
void FractionalStepAxisymmetricDFIBMultiphase::computeIbForces(
    Scalar timeStep) {
  for (auto &ibObj : *ib_) {
    if (ibObj->shape().type() != Shape2D::CIRCLE ||
        ibObj->shape().centroid().x != 0.)
      throw Exception("FractionalStepAxisymmetricDFIBMultiphase",
                      "computeIbForces",
    ...
    for (const Cell &c : ibObj->cells()) {
      ...
      for (const InteriorLink &nb : c.neighbours()) {
        Scalar flux0 = rho_(c) *
                       dot(u_.oldField(0)(nb.face()), nb.polarOutwardNorm()) /
                       2.;
        Scalar flux1 = rho_(c) *
                       dot(u_.oldField(1)(nb.face()), nb.polarOutwardNorm()) /
                       2.;
      ...
      for (const BoundaryLink &bd : c.boundaries()) {
        Scalar flux0 = rho_(c) *
                       dot(u_.oldField(0)(bd.face()), bd.polarOutwardNorm()) /
                       2.;
        Scalar flux1 = rho_(c) *
                       dot(u_.oldField(1)(bd.face()), bd.polarOutwardNorm()) /
                       2.;
        fh += std::max(flux0, 0.) * u_.oldField(0)(c) +
    ...
    for (const Cell &c : ibObj->solidCells()) {
      ...
      auto st = CelesteAxisymmetricImmersedBoundary::ContactLineStencil(
          *ibObj, c.centroid(), fst_.theta(*ibObj), gamma_);
      Scalar beta = (st.cl()[1] - ibObj->shape().centroid()).angle();
      contactLines_.emplace_back(
          ContactLine{st.cl()[1], beta, st.gamma(), st.ncl(), st.tcl()});
    }

    contactLines_ = grid_->comm().allGatherv(contactLines_);

    std::sort(contactLines_.begin(), contactLines_.end(),
              [](const ContactLine &lhs, const ContactLine &rhs) {
    ...
    for (size_t i = 0; i < contactLines_.size() - 1; ++i) {
      ...
      if ((g1 < 0.5) != (g2 <= 0.5)) {
        ...
        Vector2D tcl = alpha < 0.5 ? cl1.tcl.rotate(beta - beta1)
                                   : cl2.tcl.rotate(beta - beta2);

        Point2D pt =
            circ.centroid() + (cl1.pt - circ.centroid()).rotate(beta - beta1);
        Scalar r = pt.x;
...
void FractionalStepAxisymmetricDFIBMultiphase::computeFieldExtenstions(
    Scalar timeStep) {
  ...
  for (const auto &ibObj : *ib_)
    for (const Cell &c : ibObj->solidCells()) {
      if (extend(*ibObj, c)) {
        ...
        CelesteImmersedBoundary::ContactLineStencil cl(
            *ibObj, c.centroid(), fst_.theta(*ibObj), gamma_);
        CelesteImmersedBoundary::ContactLineStencil stn(*ibObj, c.centroid(),
                                                        M_PI_2, gamma_);
