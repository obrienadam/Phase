...
FractionalStepDirectForcingMultiphase::FractionalStepDirectForcingMultiphase(
    ...
    : FractionalStepDFIB(input, grid),
      ...
      fst_(
          std::make_shared<CelesteImmersedBoundary>(input, grid_, fluid_, ib_)),
  ...
  for (const Face &face : grid_->interiorFaces()) {
    ...
    capillaryTimeStep_ = std::min(
        capillaryTimeStep_, std::sqrt((rho1_ + rho2_) * std::pow(delta, 3) /
                                      (4. * M_PI * fst_->sigma())));
  }
...
Scalar FractionalStepDirectForcingMultiphase::solve(Scalar timeStep) {
  ...
  grid_->comm().printf(
      "Solving pressure equation and correcting velocities...\n");
  solvePEqn(timeStep);
  correctVelocity(timeStep);

  //- Perform field extensions
...
Scalar FractionalStepDirectForcingMultiphase::solveGammaEqn(Scalar timeStep) {
  ...
  gammaEqn_ =
      (fv::ddt(gamma_, timeStep) + cicsam::div(u_, gamma_, beta, 0.) == 0.);
  ...
  gammaEqn_ == cicsam::div(u_, gamma_, beta, 0.5) -
                   cicsam::div(u_, gamma_, beta, 0.) + src::src(gammaSrc_);
...
Scalar FractionalStepDirectForcingMultiphase::solveUEqn(Scalar timeStep) {
  ...
  uEqn_ = (rho_ * fv::ddt(u_, timeStep) + rho_ * fv::dive(u_, u_, 0.5) ==
           fv::laplacian(mu_, u_, 0.5) +
               src::src(fst.oldField(0) + sg_.oldField(0) - gradP_));
  ...
  for (const Cell &c : *fluid_)
    u_(c) += timeStep *
             (fb_(c) + gradP_(c) - fst.oldField(0)(c) - sg_.oldField(0)(c)) /
             rho_(c);
...
void FractionalStepDirectForcingMultiphase::solveExtEqns() {
...
void FractionalStepDirectForcingMultiphase::correctVelocity(Scalar timeStep) {
  for (const Cell &cell : *fluid_)
    u_(cell) -=
        timeStep / rho_(cell) * (gradP_(cell) - (*fst_->fst())(cell)-sg_(cell));
...
void FractionalStepDirectForcingMultiphase::computeIbForces(Scalar timeStep) {
  for (auto &ibObj : *ib_) {
    ...
    for (const Cell &c : ibObj->solidCells()) {
      ...
      CelesteImmersedBoundary::ContactLineStencil st1(
          *ibObj, pt, fst_->theta(*ibObj), gamma_);
    ...
    if (ibObj->shape().type() == Shape2D::CIRCLE) {
      ...
      for (auto i = 0; i < contactLines_.size(); ++i) {
        ...
        //                fb += Vector2D(
        //                            r*(pA*tA*sin(tA) - pA*tB*sin(tA) +
        //                            pA*cos(tA) - pA*cos(tB) - pB*tA*sin(tB) +
        //                            pB*tB*sin(tB) - pB*cos(tA) +
        //                            pB*cos(tB))/(tA - tB), r*(-pA*tA*cos(tA) +
        //                            pA*tB*cos(tA) + pA*sin(tA) - pA*sin(tB) +
        //                            pB*tA*cos(tB) - pB*tB*cos(tB) - pB*sin(tA)
        //                            + pB*sin(tB))/(tA - tB)
        //                            );
        ...
        if (gA <= 0.5 != gB < 0.5) {
          ...
          Vector2D tcl =
              alpha < 0.5 ? stA.tcl.rotate(t - tA) : stB.tcl.rotate(t - tB);
          fc += sigma * tcl;
        }
      }
    }
    ...
    for (const Cell &c : ibObj->cells()) {
      ...
      for (const InteriorLink &nb : c.neighbours()) {
        Scalar flux0 =
            rho_(c) * dot(u_.oldField(0)(nb.face()), nb.outwardNorm()) / 2.;
        Scalar flux1 =
            rho_(c) * dot(u_.oldField(1)(nb.face()), nb.outwardNorm()) / 2.;
      ...
      for (const BoundaryLink &bd : c.boundaries()) {
        Scalar flux0 =
            rho_(c) * dot(u_.oldField(0)(bd.face()), bd.outwardNorm()) / 2.;
        Scalar flux1 =
            rho_(c) * dot(u_.oldField(1)(bd.face()), bd.outwardNorm()) / 2.;
        fh += std::max(flux0, 0.) * u_.oldField(0)(c) +
