...
public:
  ...
  //- Info
  virtual std::string info() const { return "Unknown solver type"; }
  ...
  template <class T>
  std::shared_ptr<FiniteVolumeField<T>>
  addField(const std::string &name,
           const std::shared_ptr<CellGroup> &cells = nullptr);

  template <class T>
  std::shared_ptr<FiniteVolumeField<T>>
  addField(const Input &input, const std::string &name,
           const std::shared_ptr<CellGroup> &cells = nullptr);

  template <class T>
  std::shared_ptr<FiniteVolumeField<T>>
  addField(const std::shared_ptr<FiniteVolumeField<T>> &field);

  //- Field data structures
  const std::unordered_map<std::string, std::shared_ptr<FiniteVolumeField<int>>>
      &integerFields() const {
  ...
  integerField(const std::string &name) const;

  std::shared_ptr<ScalarFiniteVolumeField>
  scalarField(const std::string &name) const;

  std::shared_ptr<VectorFiniteVolumeField>
  vectorField(const std::string &name) const;
  ...
  //- IBs
  virtual std::shared_ptr<const ImmersedBoundary> ib() const { return nullptr; }

protected:
  void setCircle(const Circle &circle, Scalar innerValue,
                 ScalarFiniteVolumeField &field);

  void setCircle(const Circle &circle, const Vector2D &innerValue,
                 VectorFiniteVolumeField &field);

  void setCircleSector(const Circle &circle, Scalar thetaMin, Scalar thetaMax,
                       Scalar innerValue, ScalarFiniteVolumeField &field);

  void setBox(const Polygon &box, Scalar innerValue,
              ScalarFiniteVolumeField &field);

  void setBox(const Polygon &box, const Vector2D &innerValue,
              VectorFiniteVolumeField &field);

  void setRotating(const std::string &function, Scalar amplitude,
                   const Vector2D &center, ScalarFiniteVolumeField &field);

  void setRotating(const std::string &xFunction, const std::string &yFunction,
  ...
  mutable std::unordered_map<std::string,
                             std::shared_ptr<TensorFiniteVolumeField>>
      tensorFields_;
