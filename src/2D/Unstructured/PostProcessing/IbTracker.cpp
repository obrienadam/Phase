...
IbTracker::IbTracker(int fileWriteFreq,
                     ...
                     double lineThickness, const std::string &fillColor)
    : Object(fileWriteFreq), lineThickness_(lineThickness),
      fillColor_(fillColor), ib_(ib) {
  path_ /= "IbTracker";
    ...
    if (ib_.lock()->grid()->comm().isMainProc())
      for (const auto &ibObj : *ib_.lock()) {
        ...
        case Shape2D::CIRCLE: {
          ...
          fout << "Geometry x=" << circ.centroid().x
               << " y=" << circ.centroid().y
        ...
        case Shape2D::BOX: {
          ...
          fout << "Geometry x=" << box.lower().x << " y=" << box.lower().y
               << " T=RECTANGLE C=BLACK LT=" << lineThickness_
               << " FC=" << fillColor_ << " CS=GRID ZN=" << zoneNo_ << "\n"
               << box.upper().x - box.lower().x << " "
               << box.upper().y - box.lower().y << std::endl;
        } break;
        default: {
          auto pgn = ibObj->shape().polygonize();

          fout << "Geometry x=0 y=0 T=LINE C=BLACK LT=" << lineThickness_
               << " FC=" << fillColor_ << " CS=GRID ZN=" << zoneNo_ << "\n"
               << "1\n"
               << pgn.vertices().size() << "\n";
        ...
        fout << time << "," << ibObj->position().x << "," << ibObj->position().y
             << "," << ibObj->theta() << "," << ibObj->velocity().x << ","
             << ibObj->velocity().y << "," << ibObj->omega() << ","
