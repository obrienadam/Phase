...
ImmersedBoundaryObjectProbe::ImmersedBoundaryObjectProbe(
    int fileWriteFreq, const std::weak_ptr<const ImmersedBoundaryObject> &ibObj,
    const std::weak_ptr<const ScalarFiniteVolumeField> &field,
    const Vector2D &probePos)
    : Object(fileWriteFreq), ibObj_(ibObj), field_(field) {

  path_ /= "ImmersedBoundaryObjectProbes/" + ibObj_.lock()->name() + "/" +
           field_.lock()->name();
...
void ImmersedBoundaryObjectProbe::compute(Scalar time, bool force) {
  if (do_update() || force) {
    ...
    BilinearInterpolator bi(field->grid(),
                            ibObj_.lock()->position() + probePos_);
...
std::string ImmersedBoundaryObjectProbe::getFilename() const {
  return (path_ /
          (ibObj_.lock()->name() + "_" + field_.lock()->name() + "_probe.csv"))
      .string();
}
