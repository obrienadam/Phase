...
CompactCgnsViewer::CompactCgnsViewer(const CommandLine &cl, const Input &input,
                                     const Solver &solver)
    : Viewer(cl, input, solver), solnNo_(0) {
  ...
  if (!isRestart_) {
    filename_ =
        (path / ("proc" + std::to_string(solver.comm().rank()) + ".cgns"))
            .string();
    CgnsFile file(filename_, CgnsFile::WRITE);

    bid_ =
        file.createBase(input.caseInput().get<std::string>("CaseName"), 2, 2);
    zid_ = file.createUnstructuredZone(bid_, "Cells", solver.grid()->nNodes(),
                                       solver.grid()->nCells());
    ...
    file.writeMixedElementSection(bid_, zid_, "Elements", 1,
                                  solver.grid()->nCells(), cptr, cind);
    ...
    //            elems.insert(elems.end(), {(int)face.lNode().id() + 1,
    //            (int)face.rNode().id() + 1});

    //        int sid = file.writeBarElementSection(bid_, zid_, (patch.name() +
    //        "Elements"), start, end, elems); int bcid = file.writeBoCo(bid_,
    //        zid_, patch.name(), start, end);
...
void CompactCgnsViewer::write(Scalar time) {
  ...
  int sid = file.writeSolution(bid_, zid_,
                               "FlowSolution" + std::to_string(++solnNo_));
  file.writeDescriptorNode(bid_, zid_, sid, "SolutionTime",
                           std::to_string(time));
