...
CgnsViewer::CgnsViewer(const CommandLine &cl, const Input &input,
                       const Solver &solver)
    : Viewer(cl, input, solver) {
  boost::filesystem::path path =
      "solution/Proc" + std::to_string(solver.grid()->comm().rank());
  ...
  int zid = file.createUnstructuredZone(bid, "Zone", solver.grid()->nNodes(),
                                        solver.grid()->nCells());
  ...
  std::transform(cind.begin(), cind.end(), cind.begin(),
                 [](Label id) { return id + 1; });
  ...
  for (const FaceGroup &patch : solver.grid()->patches()) {
    ...
    for (const Face &face : patch)
      elems.insert(elems.end(),
                   {(int)face.lNode().id() + 1, (int)face.rNode().id() + 1});

    int sid = file.writeBarElementSection(bid, zid, (patch.name() + "Elements"),
                                          start, end, elems);
...
void CgnsViewer::write(Scalar time) {
  boost::filesystem::path path = "solution/" + std::to_string(time) + "/Proc" +
                                 std::to_string(solver_.grid()->comm().rank());
  ...
  int zid = file.createUnstructuredZone(bid, "Zone", solver_.grid()->nNodes(),
                                        solver_.grid()->nCells());
  ...
  file.linkNode(bid, zid, "GridCoordinates", path.c_str(),
  ...
  for (const FaceGroup &patch : solver_.grid()->patches()) {
    file.linkNode(bid, zid, (patch.name() + "Elements").c_str(), path.c_str(),
                  ("/Grid/Zone/" + patch.name() + "Elements").c_str());
  }

  file.linkNode(bid, zid, sid, "GlobalID", path.c_str(),
                "/Grid/Zone/Info/GlobalID");
  file.linkNode(bid, zid, sid, "ProcNo", path.c_str(),
                "/Grid/Zone/Info/ProcNo");
  file.close();
}
