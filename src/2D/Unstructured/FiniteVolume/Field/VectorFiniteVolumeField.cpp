...
template <> void VectorFiniteVolumeField::faceToCell(const CellGroup &cells) {
  auto &self = *this;
...
template <>
void VectorFiniteVolumeField::faceToCellAxisymmetric(
    const FiniteVolumeField<Scalar> &cw, const FiniteVolumeField<Scalar> &fw,
...
void VectorFiniteVolumeField::setBoundaryRefValues(const Input &input) {
  ...
  string valStr =
      input.boundaryInput().get<string>("Boundaries." + name_ + ".*.value", "");
  ...
  for (const FaceGroup &patch : grid()->patches()) {
    valStr = input.boundaryInput().get<string>(
        "Boundaries." + name_ + "." + patch.name() + ".value", "");
...
template <> void VectorFiniteVolumeField::setBoundaryFaces() {
  ...
  for (const FaceGroup &patch : grid_->patches())
    ...
    case SYMMETRY:
      for (const Face &face : patch)
        self(face) = self(face.lCell()) - dot(self(face.lCell()), face.norm()) *
                                              face.norm() /
                                              face.norm().magSqr();
      break;
    default:
      throw Exception("VectorFiniteVolumeField", "setBoundaryFaces",
                      "unrecognized boundary type.");
    }
}

template <> bool VectorFiniteVolumeField::isfinite() const {
  int isFinite = 1;
  for (const Cell &cell : cells())
