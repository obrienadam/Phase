...
void ScalarFiniteVolumeField::setBoundaryRefValues(const Input &input) {
  ...
  if (input.boundaryInput().count("Boundaries." + name_ + ".*") != 0) {
    Scalar refVal =
        input.boundaryInput().get<Scalar>("Boundaries." + name_ + ".*.value");
  ...
  for (const FaceGroup &patch : grid()->patches()) {
    Scalar refVal = input.boundaryInput().get<Scalar>(
        "Boundaries." + name_ + "." + patch.name() + ".value", 0);
    BoundaryType type = patchBoundaries_[patch.name()].first;
    patchBoundaries_[patch.name()] = std::make_pair(type, refVal);
  }
...
template <> bool ScalarFiniteVolumeField::isfinite() const {
  int isFinite = 1;
...
ScalarFiniteVolumeField operator*(const ScalarFiniteVolumeField &lhs,
                                  ScalarFiniteVolumeField rhs) {
  rhs *= lhs;
  return rhs;
}
...
ScalarFiniteVolumeField operator-(const ScalarFiniteVolumeField &lhs,
  ...
  std::transform(lhs.begin(), lhs.end(), diff.begin(),
                 [rhs](Scalar val) { return val - rhs; });

  return lhs;
}

ScalarFiniteVolumeField operator/(ScalarFiniteVolumeField lhs,
