...
template <class T> class FiniteVolumeField : public Field<T> {
public:
  enum BoundaryType { FIXED, NORMAL_GRADIENT, SYMMETRY, OUTFLOW, PARTIAL_SLIP };

  enum InterpolationType { VOLUME, DISTANCE };

  //- Constructors
  explicit FiniteVolumeField(
      const std::shared_ptr<const FiniteVolumeGrid2D> &grid,
      const std::string &name, const T &val = T(), bool faces = true,
      bool nodes = false,
  ...
  std::shared_ptr<IndexMap> &indexMap() { return indexMap_; }

  const std::shared_ptr<IndexMap> &indexMap() const { return indexMap_; }

  template <class TFunc> void computeCells(const TFunc &fcn) {
    for (const Cell &cell : grid_->cells())
      (*this)(cell) = fcn(cell);
  }

  template <class TFunc> void computeFaces(const TFunc &fcn) {
    for (const Face &face : grid_->faces())
      (*this)(face) = fcn(face);
  }

  template <class TFunc> void computeInteriorFaces(const TFunc &fcn) {
    for (const Face &face : grid_->interiorFaces())
      (*this)(face) = fcn(face);
  }

  template <class TFunc> void computeBoundaryFaces(const TFunc &fcn) {
  ...
  void faceToCell(const FiniteVolumeField<Scalar> &cellWeight,
                  ...
                  const CellGroup &cells, const UnaryPredicate &p);
  ...
  void faceToCellAxisymmetric(const FiniteVolumeField<Scalar> &cw,
                              const FiniteVolumeField<Scalar> &fw,
                              const CellGroup &cells);
  ...
  template <class TFunc> void interpolateFaces(const TFunc &alpha);

  void interpolateFaces(InterpolationType type = DISTANCE);

  void setBoundaryFaces();

  void setBoundaryFaces(BoundaryType bType,
                        const std::function<T(const Face &face)> &fcn);

  void interpolateNodes();

  //- Field info
  bool hasFaces() const { return !faces_.empty(); }
  ...
  T &operator()(Label id) { return std::vector<T>::operator[](id); }

  const T &operator()(Label id) const { return std::vector<T>::operator[](id); }
  ...
  FiniteVolumeField &oldField(int i) { return *previousTimeSteps_[i].second; }

  const FiniteVolumeField &oldField(int i) const {
    return *previousTimeSteps_[i].second;
  }

  Scalar oldTimeStep(int i) const { return previousTimeSteps_[i].first; }

  const FiniteVolumeField &prevIteration() const { return *previousIteration_; }
...
protected:
  ...
  //- Field history
  std::deque<std::pair<Scalar, std::shared_ptr<FiniteVolumeField<T>>>>
      previousTimeSteps_;
