...
DirectForcingImmersedBoundary::DirectForcingImmersedBoundary(
...
DirectForcingImmersedBoundary::computeForcingTerm(
  ...
  for (const Cell &cell : u.cells()) {
    if (localIbCells_.isInSet(cell)) {
      auto st = DirectForcingImmersedBoundary::LeastSquaresQuadraticStencil(
          cell, *this);
      ...
      for (const Face *facePtr : st.faces())
        ...
        default:
          throw Exception("DirectForcingImmersedBoundary", "velocityBcs",
                          "grid boundary type not recognized.");
        }
...
DirectForcingImmersedBoundary::computeForcingTerm(
    ...
    Scalar timeStep, VectorFiniteVolumeField &fib) const {
  FiniteVolumeEquation<Vector2D> eqn(fib, 12);
  for (const Cell &cell : u.cells()) {
    if (localIbCells_.isInSet(cell)) {
      auto st = DirectForcingImmersedBoundary::LeastSquaresQuadraticStencil(
      ...
      for (const Face *facePtr : st.faces())
        ...
        case VectorFiniteVolumeField::SYMMETRY: {
          ...
          Vector2D t = n.tangentVec();
          eqn.add(cell, cell,
                  beta(0, i) * timeStep * outer(t, t) / rho(*facePtr));
          eqn.addSource(cell, beta(0, i++) * dot(u(cell), t) * t);
          break;
        }
        default:
...
FiniteVolumeEquation<Vector2D>
DirectForcingImmersedBoundary::computeFieldExtension(
...
FiniteVolumeEquation<Vector2D>
DirectForcingImmersedBoundary::computeFieldExtension(
    const ScalarFiniteVolumeField &rho, const VectorFiniteVolumeField &sg,
    VectorFiniteVolumeField &gradP) const {
  FiniteVolumeEquation<Vector2D> eqn(gradP, 12);
  ...
  for (const Cell &cell : gradP.cells()) {
    ...
    if (localIbCells_.isInSet(cell)) {
      ...
      for (const auto &cmpt : st.compatPts())
        eqn.addSource(cell, beta(0, i++) *
                                (-rho(cell) * cmpt.acceleration() + sg(cell)));
...
FiniteVolumeEquation<Vector2D> DirectForcingImmersedBoundary::velocityBcs(
    VectorFiniteVolumeField &u, const VectorFiniteVolumeField &uTilde,
  ...
  for (const Cell &cell : u.cells()) {
    if (localIbCells_.isInSet(cell)) {
      ...
      for (const auto &cmpt : st.compatPts())
        eqn.addSource(cell, beta(0, i++) * cmpt.velocity() * cell.volume() /
                                timeStep);
      ...
      for (const Face *facePtr : st.faces())
        ...
        case VectorFiniteVolumeField::FIXED:
          eqn.addSource(cell,
                        beta(0, i++) * u(*facePtr) * cell.volume() / timeStep);
          break;
        case VectorFiniteVolumeField::SYMMETRY: {
          Vector2D n = facePtr->norm().unitVec();
          Vector2D t = n.tangentVec();
      ...
      eqn.addSource(cell,
                    (ibObj(cell)->velocity(cell.centroid()) - uTilde(cell)) *
                        cell.volume() / timeStep);
  }
...
FiniteVolumeEquation<Vector2D>
DirectForcingImmersedBoundary::continuityVelocityBcs(
    VectorFiniteVolumeField &u, const VectorFiniteVolumeField &uTilde,
    Scalar timeStep) const {
  FiniteVolumeEquation<Vector2D> eqn(u, 20);

  for (const Cell &cell : u.cells()) {
    if (localIbCells_.isInSet(cell)) {
      auto st = DirectForcingImmersedBoundary::LeastSquaresQuadraticStencil(
          cell, *this);
      auto beta = st.continuityConstrainedInterpolationCoeffs(cell.centroid());

      // std::cout << beta.m() << " " << beta.n() << std::endl;

      int i = 0;
      for (const Cell *cellPtr : st.cells()) {
        eqn.add(
            cell, *cellPtr,
            Tensor2D(beta(0, i), beta(0, i + 1), beta(1, i), beta(1, i + 1)) *
                cell.volume() / timeStep);
        i += 2;
      }

      for (const auto &cmpt : st.compatPts()) {
        eqn.addSource(cell, dot(Tensor2D(beta(0, i), beta(0, i + 1), beta(1, i),
                                         beta(1, i + 1)),
                                cmpt.velocity()) *
                                cell.volume() / timeStep);
      ...
      for (const Face *facePtr : st.faces())
        switch (u.boundaryType(*facePtr)) {
        case VectorFiniteVolumeField::FIXED:
          eqn.addSource(cell, dot(Tensor2D(beta(0, i), beta(0, i + 1),
                                           beta(1, i), beta(1, i + 1)),
                                  u(*facePtr)) *
                                  cell.volume() / timeStep);
          break;
          ...
          //                    Vector2D t = n.tangentVec();
          //                    Tensor2D tmp = outer(t, t);
          //                    eqn.add(cell, cell, beta(0, i++) * tmp *
          //                    cell.volume() / timeStep); break;
          //                }
          //                case VectorFiniteVolumeField::NORMAL_GRADIENT:
          //                    eqn.add(cell, cell, beta(0, i++) * cell.volume()
          //                    / timeStep); break;
        default:
          throw Exception("DirectForcingImmersedBoundary", "velocityBcs",
                          "grid boundary type not recognized.");
...
FiniteVolumeEquation<Vector2D> DirectForcingImmersedBoundary::polarVelocityBcs(
    VectorFiniteVolumeField &u, const VectorFiniteVolumeField &uTilde,
    Scalar timeStep) const {
  FiniteVolumeEquation<Vector2D> eqn(u, 10);
    ...
    if (localIbCells_.isInSet(cell)) {
      ...
      for (const Face *facePtr : st.faces())
        ...
        case VectorFiniteVolumeField::NORMAL_GRADIENT:
          ...
          break;
        default:
          throw Exception("DirectForcingImmersedBoundary", "polarVelocityBcs",
                          "grid boundary type not recognized.");
        }
...
FiniteVolumeEquation<Vector2D>
DirectForcingImmersedBoundary::continuityPolarVelocityBcs(
    VectorFiniteVolumeField &u, const VectorFiniteVolumeField &uTilde,
    Scalar timeStep) const {
  FiniteVolumeEquation<Vector2D> eqn(u, 20);
  ...
  for (const Cell &cell : u.cells()) {
    if (localIbCells_.isInSet(cell)) {
      auto st = DirectForcingImmersedBoundary::LeastSquaresQuadraticStencil(
          cell, *this);
      auto beta = st.polarQuadraticContinuityConstrainedInterpolationCoeffs(
          cell.centroid());
      ...
      eqn.addSource(cell, -uTilde(cell) * cell.polarVolume() / timeStep);
    } else if (localSolidCells_.isInSet(cell))
      eqn.addSource(cell,
                    (ibObj(cell)->velocity(cell.centroid()) - uTilde(cell)) *
                        cell.polarVolume() / timeStep);
...
void DirectForcingImmersedBoundary::applyHydrodynamicForce(
  ...
  for (auto &ibObj : ibObjs_) {
    ...
    auto indexStart =
        6 * std::accumulate(nLocalCells.begin(),
    ...
    for (const Cell &cell : ibObj->ibCells()) {
      ...
      Matrix A(st.nReconstructionPoints(), 6),
          rhs(st.nReconstructionPoints(), 2);
      ...
      auto derivs = Matrix(2, 6,
                           {2. * xb.x, 0., xb.y, 1., 0., 0., 0., 2. * xb.y,
                            xb.x, 0., 1., 0.}) *
                    coeffs;

      //- The tensor is tranposed here
      auto tau =
          Tensor2D(derivs(0, 0), derivs(1, 0), derivs(0, 1), derivs(1, 1));
      tau = mu * (tau + tau.transpose());

      stresses.push_back(Stress{xb, 0., tau});

      auto divTau =
          mu * Vector2D(4 * coeffs(0, 0) + 2 * coeffs(1, 0) + coeffs(2, 1),
      ...
      for (const Cell *cell : st.cells()) {
        ...
        eqn.setCoeffs(row,
                      {colStart, colStart + 1, colStart + 2, colStart + 3,
                       colStart + 4, colStart + 5},
      ...
      for (const auto &compatPt : st.compatPts()) {
        ...
        eqn.setCoeffs(row++,
                      {colStart2, colStart2 + 1, colStart2 + 2, colStart2 + 3,
                       colStart2 + 4, colStart2 + 5},
                      {-x.x * x.x, -x.y * x.y, -x.x * x.y, -x.x, -x.y, -1.});
      }
      ...
      eqn.setCoeffs(row,
                    {colStart, colStart + 1, colStart + 2, colStart + 3,
                     colStart + 4, colStart + 5},
                    {2. * xb.x * n.x, 2. * xb.y * n.y, xb.y * n.x + xb.x * n.y,
                     n.x, n.y, 0.});
    ...
    for (int i = 0; i < 6 * ibObj->ibCells().size(); i += 6) {
      ...
      stress.p = a * xb.x * xb.x + b * xb.y * xb.y + c * xb.x * xb.y +
                 d * xb.x + e * xb.y + f + rho * dot(xb, g);
    }
    ...
    if (grid_->comm().isMainProc()) {
      ...
      std::sort(stresses.begin(), stresses.end(),
                [&ibObj](const Stress &lhs, const Stress &rhs) {
      ...
      for (int i = 0; i < stresses.size(); ++i) {
        ...
        if (ibObj->shape().type() == Shape2D::CIRCLE) {
          ...
          fPressure +=
              r *
              Vector2D(pA * tA * sin(tA) - pA * tB * sin(tA) + pA * cos(tA) -
                           pA * cos(tB) - pB * tA * sin(tB) +
                           pB * tB * sin(tB) - pB * cos(tA) + pB * cos(tB),
                       -pA * tA * cos(tA) + pA * tB * cos(tA) + pA * sin(tA) -
                           pA * sin(tB) + pB * tA * cos(tB) -
                           pB * tB * cos(tB) - pB * sin(tA) + pB * sin(tB)) /
              (tA - tB);

          fShear +=
              r *
              Vector2D(-tA * tauA.xx * sin(tA) + tA * tauA.xy * cos(tA) +
                           tA * tauB.xx * sin(tB) - tA * tauB.xy * cos(tB) +
                           tB * tauA.xx * sin(tA) - tB * tauA.xy * cos(tA) -
                           tB * tauB.xx * sin(tB) + tB * tauB.xy * cos(tB) -
                           tauA.xx * cos(tA) + tauA.xx * cos(tB) -
                           tauA.xy * sin(tA) + tauA.xy * sin(tB) +
                           tauB.xx * cos(tA) - tauB.xx * cos(tB) +
                           tauB.xy * sin(tA) - tauB.xy * sin(tB),
                       -tA * tauA.yx * sin(tA) + tA * tauA.yy * cos(tA) +
                           tA * tauB.yx * sin(tB) - tA * tauB.yy * cos(tB) +
                           tB * tauA.yx * sin(tA) - tB * tauA.yy * cos(tA) -
                           tB * tauB.yx * sin(tB) + tB * tauB.yy * cos(tB) -
                           tauA.yx * cos(tA) + tauA.yx * cos(tB) -
                           tauA.yy * sin(tA) + tauA.yy * sin(tB) +
                           tauB.yx * cos(tA) - tauB.yx * cos(tB) +
                           tauB.yy * sin(tA) - tauB.yy * sin(tB)) /
              (tA - tB);
        } else {
    ...
    ibObj->applyForce(
        grid_->comm().broadcast(grid_->comm().mainProcNo(), force));
  }
}
...
void DirectForcingImmersedBoundary::applyHydrodynamicForce(
  ...
  for (auto &ibObj : ibObjs_) {
    ...
    for (const Cell &cell : ibObj->ibCells()) {
      ...
      auto divTau =
          mu(cell) *
          Vector2D(4 * coeffs(0, 0) + 2 * coeffs(1, 0) + coeffs(2, 1),
                   2 * coeffs(0, 1) + 4 * coeffs(1, 1) + coeffs(2, 0));
      ...
      eqn.setRhs(row++,
                 rho(cell) * dot(ibObj->acceleration(xb), n) - dot(divTau, n));
    }
    ...
    for (int i = 0; i < 6 * ibObj->ibCells().size(); i += 6) {
      ...
      Point2D xb = stress.pt;
      stress.p = a * xb.x * xb.x + b * xb.y * xb.y + c * xb.x * xb.y +
                 d * xb.x + e * xb.y + f + rho(cell) * dot(xb, g);
    }
...
void DirectForcingImmersedBoundary::applyHydrodynamicForce(
    Scalar rho, const VectorFiniteVolumeField &fib, const Vector2D &g) {
...
//    auto colOwnershipRange = std::make_pair(6 *
//    std::accumulate(nLocalIbCells.begin(), nLocalIbCells.begin() +
//    grid_->comm().rank(), 0),
//                                            6 *
//                                            std::accumulate(nLocalIbCells.begin(),
//                                            nLocalIbCells.begin() +
//                                            grid_->comm().rank() + 1, 0));
...
//                    compatPts.push_back(std::make_pair(&nb.cell(),
//                    nearestIntersect(nb.cell().centroid())));
//            }
...
//            eqn.setRhs(row++, -(p(*cellPtr) + rho(*cellPtr) * dot(g,
//            cellPtr->centroid())));
//        }
...
//        eqn.setCoeffs(row,
//        {col, col + 1, col + 2, col + 3, col + 4, col + 5},
//        {2. * x.x * n.x, 2. * x.y * n.y, x.y * n.x + x.x * n.y, n.x, n.y,
//        0.});
...
//    Tpetra::DynamicProfile)); eqn.setRank(eqn.rank(), 6 * ibCells_.size());
//    eqn.solveLeastSquares();
...
//        Scalar pb = a * x.x * x.x + b * x.y * x.y + c * x.x * x.y + d * x.x +
//        e * x.y + f;
...
//            if(!solidCells.isInGroup(nb.cell()))
//            {
//                pts.push_back(std::make_pair(nb.cell().centroid(),
//                u(nb.cell())));
...
//        auto tau = Tensor2D(derivs(0, 0), derivs(1, 0), derivs(0, 1),
//        derivs(1, 1));

//        stresses.push_back(std::make_tuple(x, pb, mu(cell) * (tau +
//        tau.transpose())));
//    }
...
//        std::sort(stresses.begin(), stresses.end(), [this](std::tuple<Point2D,
//        Scalar, Tensor2D> &lhs,
