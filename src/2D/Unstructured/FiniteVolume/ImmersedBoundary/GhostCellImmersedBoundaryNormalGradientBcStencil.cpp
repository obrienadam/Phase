...
GhostCellImmersedBoundary::NormalGradientBcStencil::NormalGradientBcStencil(
...
void GhostCellImmersedBoundary::NormalGradientBcStencil::
    initInterpolationCoeffs(const ImmersedBoundaryObject &ibObj,
                            const FiniteVolumeGrid2D &grid) {
  ...
  if (cellInStencil) {
    coeffs_ =
        StaticMatrix<1, 4>({bp_.y * nw_.x + bp_.x * nw_.y, nw_.x, nw_.y, 0.}) *
        Ainv_;
  } else {
    ...
    Scalar l = length();
    coeffs_ =
        -StaticMatrix<1, 4>({ip_.x * ip_.y, ip_.x, ip_.y, 1.}) * Ainv_ / l;
