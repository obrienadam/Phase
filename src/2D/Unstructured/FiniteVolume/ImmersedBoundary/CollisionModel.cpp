...
Vector2D CollisionModel::force(const ImmersedBoundaryObject &ibObjP,
                               const ImmersedBoundaryObject &ibObjQ) const {
  if (ibObjP.shape().type() == Shape2D::CIRCLE &&
      ibObjQ.shape().type() == Shape2D::CIRCLE) {
    ...
    return d > r1 + r2 + range_
               ? Vector2D(0., 0.)
               : (xp - xq) / eps_ * std::pow(r1 + r2 + range_ - d, 2);
  } else
    throw Exception("CollisionModel", "force", "unsupported shape type.");
}

Vector2D CollisionModel::force(const ImmersedBoundaryObject &ibObj,
