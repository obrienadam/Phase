...
template <class T> class SurfaceField {
public:
  SurfaceField(const std::shared_ptr<const ImmersedBoundaryObject> &ibObj);

  Size size() const { return _field.size(); }

  const Point2D &pt(Label i) const { return _field[i].first; }

  T &operator()(Label i) { return _field[i].second; }

  const T &operator()(Label i) const { return _field[i].second; }
