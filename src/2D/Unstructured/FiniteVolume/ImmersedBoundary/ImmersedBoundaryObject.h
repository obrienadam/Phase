...
public:
  enum BoundaryConditionType { FIXED, NORMAL_GRADIENT, VELOCITY };
  ...
  std::vector<Ref<const Cell>>
  internalPerimeterCells(const_iterator first, const_iterator last,
                         bool includeDiagonals = false) const;

  template <class const_iterator>
  std::vector<Ref<const Cell>>
  outerPerimeterCells(const_iterator first, const_iterator last,
                      bool includeDiagonals = false) const;

  //- Cell groups
  const CellGroup &ibCells() const { return _ibCells; }

  const CellGroup &solidCells() const { return _solidCells; }
  ...
  const Shape2D &shape() const { return *_shape; }

  //- Tests
  bool isInIb(const Point2D &pt) const { return _shape->isInside(pt); }

  bool isInIb(const Cell &cell) const { return isInIb(cell.centroid()); }
  ...
  //- Boundary methods
  template <class T>
  void addBoundaryCondition(const std::string &name,
                            BoundaryConditionType bcType, T bcRefValue);

  template <class T>
  void addBoundaryCondition(const std::string &name, const std::string &bcType,
                            T bcRefValue);

  template <class T>
  void addBoundaryCondition(const std::string &name, const std::string &bcType,
                            const std::string &bcRefValue);

  BoundaryConditionType bcType(const std::string &name) const;

  template <class T> T bcRefValue(const std::string &name) const;

  //- Reference to cell zone for iterating

  const CellGroup &cells() const { return _cells; }

  //- Motion info if applicable

  void setMotion(const std::shared_ptr<Motion> &motion) { _motion = motion; }

  std::shared_ptr<Motion> &motion() { return _motion; }

  std::shared_ptr<const Motion> motion() const { return _motion; }

  bool isMoving() const { return (bool)_motion; }
  ...
  Vector2D velocity() const { return velocity(position()); }

  Vector2D acceleration() const { return acceleration(position()); }
  ...
  Scalar mass() const { return rho * _shape->area(); }

  Scalar momentOfInertia() const { return rho * _shape->momentOfInertia(); }
  ...
  Scalar torque() const { return _torque; }
