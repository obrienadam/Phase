...
Scalar cicsam::hc(Scalar gammaDTilde, Scalar coD) {
  return gammaDTilde >= 0 && gammaDTilde <= 1 ? std::min(1., gammaDTilde / coD)
                                              : gammaDTilde;
}

Scalar cicsam::uq(Scalar gammaDTilde, Scalar coD) {
  return gammaDTilde >= 0 && gammaDTilde <= 1
             ? std::min((8. * coD * gammaDTilde +
                         (1. - coD) * (6. * gammaDTilde + 3.)) /
                            8.,
                        hc(gammaDTilde, coD))
             : gammaDTilde;
}

std::vector<Scalar> cicsam::faceInterpolationWeights(
  ...
  for (const Face &face : gamma.grid()->interiorFaces()) {
    ...
    Scalar thetaF =
        std::acos(std::abs(dot(gradGamma(donor).unitVec(), rc.unitVec())));
    Scalar psiF = std::min(k * (std::cos(2 * thetaF) + 1.) / 2., 1.);
    Scalar gammaFTilde =
        psiF * hc(gammaDTilde, coD) + (1. - psiF) * uq(gammaDTilde, coD);
...
void cicsam::computeMomentumFlux(
  ...
  rhoU.computeInteriorFaces(
      [rho1, rho2, &u, &gamma, &faceInterpolationWeights](const Face &f) {
...
cicsam::div(const VectorFiniteVolumeField &u, ScalarFiniteVolumeField &gamma,
  ...
  for (const Cell &cell : cells) {
    for (const InteriorLink &nb : cell.neighbours()) {
      ...
      //- Note, this weight is only an approximation of the correct implicit
      //weight
