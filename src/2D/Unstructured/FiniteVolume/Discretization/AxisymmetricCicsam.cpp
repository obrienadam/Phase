...
std::vector<Scalar> axi::cicsam::faceInterpolationWeights(
  ...
  for (const Face &face : gamma.grid()->interiorFaces()) {
    ...
    // const Cell &upwind = gamma.grid()->globalCells().nearestItem(2. *
    // donor.centroid() - acceptor.centroid());
    ...
    for (const InteriorLink &nb : d.neighbours())
      coD += std::max(dot(u(nb.face()), nb.polarOutwardNorm()) /
                          d.polarVolume() * timeStep,
                      0.);

    for (const BoundaryLink &bd : d.boundaries())
      coD += std::max(dot(u(bd.face()), bd.polarOutwardNorm()) /
                          d.polarVolume() * timeStep,
                      0.);

    Scalar thetaF =
        std::acos(std::abs(dot(gradGamma(d).unitVec(), rc.unitVec())));
    Scalar psiF = std::min(k * (std::cos(2 * thetaF) + 1.) / 2., 1.);
    Scalar gammaFTilde = psiF * ::cicsam::hc(gammaDTilde, coD) +
                         (1. - psiF) * ::cicsam::uq(gammaDTilde, coD);
...
axi::cicsam::div(const VectorFiniteVolumeField &u,
                 ScalarFiniteVolumeField &gamma,
                 const std::vector<Scalar> &faceInterpolationWeights,
                 Scalar theta, const CellGroup &cells) {
  ...
  for (const Cell &cell : gamma.cells()) {
    for (const InteriorLink &nb : cell.neighbours()) {
      ...
      eqn.addSource(cell, flux * ((1. - b) * gamma0(d) + b * gamma0(a)) *
                              (1. - theta));
    }
    ...
    for (const BoundaryLink &bd : cell.boundaries()) {
      ...
      case ScalarFiniteVolumeField::NORMAL_GRADIENT:
        break;
      default:
        throw Exception("axi::cicsam", "div",
                        "unrecognized boundary condition type.");
      }
    }
  }
...
void axi::cicsam::computeMomentumFlux(
    Scalar rho1, Scalar rho2, const VectorFiniteVolumeField &u,
...
FiniteVolumeEquation<Scalar> axi::cicsam::div(
    const VectorFiniteVolumeField &u, ScalarFiniteVolumeField &gamma,
