...
axi::laplacian(const ScalarFiniteVolumeField &gamma,
               ScalarFiniteVolumeField &phi) {
...
axi::laplacian(Scalar gamma, VectorFiniteVolumeField &u, Scalar theta) {
  ...
  for (const Cell &cell : u.cells()) {
    ...
    for (const BoundaryLink &bd : cell.boundaries()) {
      ...
      default:
        throw Exception("axi", "vectorLaplacian",
                        "unrecognized or unspecified boundary type.");
      }
    }
    ...
    eqn.addSource(cell, -gamma * cell.volume() * u0(cell).x / r *
                            Vector2D(1. - theta, 0.));
  }
...
FiniteVolumeEquation<Vector2D>
axi::laplacian(const ScalarFiniteVolumeField &gamma, VectorFiniteVolumeField &u,
               Scalar theta) {
  FiniteVolumeEquation<Vector2D> eqn(u);
  ...
  for (const Cell &cell : u.cells()) {
    ...
    eqn.addSource(cell, -gamma0(cell) * cell.volume() * u0(cell).x / r *
                            Vector2D(1. - theta, 0.));
  }
