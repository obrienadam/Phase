...
Celeste::Stencil::Stencil(const Cell &cell, bool weighted) : cellPtr_(&cell) {
  init(weighted);
}
...
Scalar Celeste::Stencil::axiDiv(const VectorFiniteVolumeField &u) const {
  ...
  for (const Cell &kCell : cells_) {
    ...
    Vector2D du = (Vector2D(kCell.centroid().x * u(kCell).x, u(kCell).y) -
                   Vector2D(cell.centroid().x * u(cell).x, u(cell).y)) /
                  s;
    b_(i, 0) = du.x;
    b_(i++, 1) = du.y;
  }
...
void Celeste::Stencil::initMatrix() {
  ...
  for (const Cell &kCell : cells_) {
    ...
    A.setRow(i++, {r.x * r.x / 2. / s, r.y * r.y / 2. / s, r.x * r.y / s,
                   r.x / s, r.y / s});
  }
