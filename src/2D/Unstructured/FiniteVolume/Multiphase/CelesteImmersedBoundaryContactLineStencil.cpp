...
std::queue<Ref<const Cell>>
    CelesteImmersedBoundary::ContactLineStencil::cellQueue_;

std::unordered_set<Label>
    CelesteImmersedBoundary::ContactLineStencil::cellIdSet_;

CelesteImmersedBoundary::ContactLineStencil::ContactLineStencil(
    const ImmersedBoundaryObject &ibObj, const Point2D &pt, Scalar theta,
    const ScalarFiniteVolumeField &gamma)
    : ibObj_(&ibObj), cellA_(nullptr), cellB_(nullptr), face_(nullptr),
      theta_(theta) {
  cl_[0] = pt;
  ...
  if (theta_ == M_PI_2)
    ...
    init(Ray2D(cl_[0], ns_.rotate(M_PI_2 - theta_)),
         Ray2D(cl_[0], ns_.rotate(theta_ - M_PI_2)), gamma);
}
...
void CelesteImmersedBoundary::ContactLineStencil::init(
  ...
  if (!isValid()) {
    ...
    throw Exception("CelesteImmersedBoundary::ContactLineStencil", "init",
                    os.str());
  }
  ...
  if (cellA_ && cellB_)
    nl = (gamma(*cellA_) - gamma(*cellB_)) *
         (cellB_->centroid() - cellA_->centroid());
  else if (cellA_ && face_)
    nl = (gamma(*cellA_) - gamma(*face_)) *
         (face_->centroid() - cellA_->centroid());
...
void CelesteImmersedBoundary::ContactLineStencil::init(
    const Ray2D &r1, const Ray2D &r2, const ScalarFiniteVolumeField &gamma) {
  findStencilCells(r1);
  ...
  if (c1.isValid() && c2.isValid()) {
    ...
    std::cout << "Warning, failed to find valid contact line stencil on proc "
              << gamma.grid()->comm().rank()
              << ". Initiating normal stencil stencil...\n";
    init(gamma);
  }
}

void CelesteImmersedBoundary::ContactLineStencil::findStencilCells(
    const Ray2D &r) {
  ...
  while (!cellQueue_.empty()) {
    ...
    for (const CellLink &nb : cell.cellLinks()) {
      if (cellIdSet_.find(nb.cell().id()) == cellIdSet_.end() &&
          !foundIntersection) // don't add to queue if a result has been found
      {
