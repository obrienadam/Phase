...
CelesteImmersedBoundary::CelesteImmersedBoundary(
    ...
    : Celeste(input, grid, fluidCells), ib_(ib) {
  for (const auto &ibObj : *ib_.lock()) {
    ibContactAngles_[ibObj->name()] =
  ...
  auto ibInput =
      input.boundaryInput().get_child_optional("ImmersedBoundaryGeometryFile");

  if (ibInput) {
    std::string filename = ibInput.get().get<std::string>("filename");
    Scalar theta = ibInput.get().get<Scalar>("fields.gamma.contactAngle", 90.) *
                   M_PI / 180.;
...
void CelesteImmersedBoundary::computeFaceInterfaceForces(
    const ScalarFiniteVolumeField &gamma, const ScalarGradient &gradGamma) {
  Celeste::computeFaceInterfaceForces(gamma, gradGamma);
}

Scalar
CelesteImmersedBoundary::theta(const ImmersedBoundaryObject &ibObj) const {
  auto it = ibContactAngles_.find(ibObj.name());
  return it != ibContactAngles_.end() ? it->second : M_PI_2;
}

void CelesteImmersedBoundary::computeContactLineExtension(
    ScalarFiniteVolumeField &gamma) const {
  for (const std::shared_ptr<ImmersedBoundaryObject> &ibObj : *ib_.lock())
    for (const Cell &cell : ibObj->cells())
      if (ibObj->isInIb(cell.centroid())) {
        Scalar distSqr =
            (ibObj->nearestIntersect(cell.centroid()) - cell.centroid())
                .magSqr();

        if (distSqr <= kernelWidth_ * kernelWidth_) {
          ContactLineStencil st(*ibObj, cell.centroid(),
...
CelesteImmersedBoundary::ContactLineStencil
CelesteImmersedBoundary::contactLineStencil(
    const Point2D &xc, const ScalarFiniteVolumeField &gamma) const {
  auto ibObj = ib_.lock()->ibObj(xc);

  if (!ibObj)
    throw Exception("CelesteImmersedBoundary", "contactLineStencil",
                    "no suitable ib found.");

  return ContactLineStencil(*ibObj, xc, theta(*ibObj), gamma);
}

void CelesteImmersedBoundary::computeContactLineStencils(
  ...
  for (const std::shared_ptr<ImmersedBoundaryObject> &ibObj : *ib_.lock())
    for (const Cell &cell : ibObj->cells())
      if (ibObj->isInIb(cell.centroid())) {
        ...
        if (distSqr <= kernelWidth_ * kernelWidth_) {
          contactLineStencils_.emplace_back(
              *ibObj, cell.centroid(),
              ibContactAngles_.find(ibObj->name())->second, gamma);
...
void CelesteImmersedBoundary::applyFluidForces(
  ...
  for (auto &ibObj : ib) {
    ...
    for (const Cell &cell : ibObj->ibCells()) {
      auto st =
          DirectForcingImmersedBoundary::LeastSquaresQuadraticStencil(cell, ib);
      ...
      auto clst = ContactLineStencil(*ibObj, xb, theta(*ibObj), gamma);
      ...
      stresses.push_back(
          Stress{xb, clst.interpolate(rho), 0., clst.gamma(), clst.tcl(), tau});

      auto divTau =
          mub * Vector2D(4 * coeffs(0, 0) + 2 * coeffs(1, 0) + coeffs(2, 1),
      ...
      eqn.setRhs(row++, stresses.back().rho * dot(ibObj->acceleration(xb), n) -
                            dot(divTau, n));
    }
    ...
    if (grid_->comm().isMainProc()) {
      ...
      for (int i = 0; i < stresses.size(); ++i) {
        ...
        if (ibObj->shape().type() == Shape2D::CIRCLE) {
          ...
          if (gA < 0.5 != gB <= 0.5) {
            ...
            Vector2D t1 = Vector2D(std::cos(phi - M_PI_2 + theta),
                                   std::sin(phi - M_PI_2 + theta));
            Vector2D t2 = Vector2D(std::cos(phi + M_PI_2 - theta),
                                   std::sin(phi + M_PI_2 - theta));

            fCapillary +=
                sigma_ * (dot(t1, qpa.tcl) > dot(t2, qpa.tcl) ? t1 : t2);
          }

          // - diffuse method

          //                    Vector2D t1 = Vector2D(std::cos(tA - M_PI_2 +
          //                    theta), std::sin(tA - M_PI_2 + theta)); Vector2D
          //                    t2 = Vector2D(std::cos(tA + M_PI_2 - theta),
          //                    std::sin(tA + M_PI_2 - theta));

          //                    if(dot(t1, qpa.tcl) > dot(t2, qpa.tcl))
          //                    {
          //                        fCapillary += Vector2D(
          //                                    dgamma*sigma_*(cos(tA + theta) -
          //                                    cos(tB + theta)),
          //                                    dgamma*sigma_*(sin(tA + theta) -
          //                                    sin(tB + theta))
          //                                    );
          //                    }
          //                    else
      ...
      force = fPressure + fShear + fCapillary +
              ibObj->rho * g * ibObj->shape().area();
...
void CelesteImmersedBoundary::applyFluidForces(
    const ScalarFiniteVolumeField &rho, const VectorFiniteVolumeField &fb,
    const ScalarFiniteVolumeField &gamma, const Vector2D &g,
    DirectForcingImmersedBoundary &ib) const {
  struct Stress {
    Point2D pt;
  ...
  for (auto &ibObj : ib) {
    ...
    for (const Cell &c : ibObj->ibCells()) {
      ...
      stresses.push_back(
          Stress{bp, th, cl.interpolate(rho), cl.gamma(), cl.tcl()});
    }
    ...
    case Shape2D::CIRCLE: {
      ...
      for (auto i = 0; i < stresses.size(); ++i) {
        ...
        fb -=
            r *
            Vector2D(pA * thA * sin(thA) - pA * thB * sin(thA) + pA * cos(thA) -
                         pA * cos(thB) - pB * thA * sin(thB) +
                         pB * thB * sin(thB) - pB * cos(thA) + pB * cos(thB),
                     -pA * thA * cos(thA) + pA * thB * cos(thA) +
                         pA * sin(thA) - pA * sin(thB) + pB * thA * cos(thB) -
                         pB * thB * cos(thB) - pB * sin(thA) + pB * sin(thB)) /
            (thA - thB);
    ...
    default:
      throw Exception("CelesteImmersedBoundary", "applyFluidForces",
