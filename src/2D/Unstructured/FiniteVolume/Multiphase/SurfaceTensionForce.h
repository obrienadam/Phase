...
public:
  ...
  const std::shared_ptr<VectorFiniteVolumeField> &fst() const { return fst_; }
  ...
  const std::shared_ptr<VectorFiniteVolumeField> &n() const { return n_; }
  ...
  //- Helpers
  Scalar dynamicContactAngle(Scalar thetaApp, Scalar Ca, Scalar delta,
                             Scalar K = 0.02) const;

  //- Compute
  virtual void computeFaceInterfaceForces(const ScalarFiniteVolumeField &gamma,
                                          const ScalarGradient &gradGamma) = 0;
