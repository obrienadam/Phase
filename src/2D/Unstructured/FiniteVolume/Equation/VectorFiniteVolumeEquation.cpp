...
template <>
FiniteVolumeEquation<Vector2D>::FiniteVolumeEquation(
    VectorFiniteVolumeField &field, const std::string &name, int nnz)
    : CrsEquation(2 * field.grid()->localCells().size(), nnz), name(name),
      field_(field) {}

template <>
void FiniteVolumeEquation<Vector2D>::set(const Cell &cell, const Cell &nb,
                                         Scalar val) {
  setCoeff(field_.indexMap()->local(cell, 0), field_.indexMap()->global(nb, 0),
           val);

  setCoeff(field_.indexMap()->local(cell, 1), field_.indexMap()->global(nb, 1),
           val);
}

template <>
void FiniteVolumeEquation<Vector2D>::add(const Cell &cell, const Cell &nb,
                                         Scalar val) {
  addCoeff(field_.indexMap()->local(cell, 0), field_.indexMap()->global(nb, 0),
           val);

  addCoeff(field_.indexMap()->local(cell, 1), field_.indexMap()->global(nb, 1),
           val);
}
...
void FiniteVolumeEquation<Vector2D>::add(const Cell &cell, const Cell &nb,
  ...
  if (val.xy != 0.)
    addCoeff(field_.indexMap()->local(cell, 0),
             field_.indexMap()->global(nb, 1), val.xy);

  if (val.yx != 0.)
    addCoeff(field_.indexMap()->local(cell, 1),
             field_.indexMap()->global(nb, 0), val.yx);
...
template <> void FiniteVolumeEquation<Vector2D>::remove(const Cell &cell) {
  Index rowX = field_.indexMap()->local(cell, 0);
  Index rowY = field_.indexMap()->local(cell, 1);

  std::fill(colInd_.begin() + rowPtr_[rowX],
            colInd_.begin() + rowPtr_[rowX + 1], -1);

  std::fill(colInd_.begin() + rowPtr_[rowY],
            colInd_.begin() + rowPtr_[rowY + 1], -1);
...
template <>
template <>
void FiniteVolumeEquation<Vector2D>::addSource(const Cell &cell,
                                               const Vector2D &u) {
...
template <>
template <>
void FiniteVolumeEquation<Vector2D>::setSource(const Cell &cell,
                                               const Vector2D &u) {
  rhs_(field_.indexMap()->local(cell, 0)) = u.x;
  rhs_(field_.indexMap()->local(cell, 1)) = u.y;
}
...
void FiniteVolumeEquation<Vector2D>::relax(Scalar relaxationFactor) {
  ...
  //    {
  //        Scalar &coeffX = coeffRef(field_.indexMap()->local(cell, 0),
  //        field_.indexMap()->global(cell, 0)); Scalar &coeffY =
  //        coeffRef(field_.indexMap()->local(cell, 1),
  //        field_.indexMap()->global(cell, 1));

  //        coeffX /= relaxationFactor;
  //        coeffY /= relaxationFactor;

  //        _rhs(field_.indexMap()->local(cell, 0)) -= (1. - relaxationFactor) *
  //        coeffX * field_(cell).x; _rhs(field_.indexMap()->local(cell, 1)) -=
  //        (1. - relaxationFactor) * coeffY * field_(cell).y;
  //    }
}

//- Private
template <> void FiniteVolumeEquation<Vector2D>::mapFromSparseSolver() {
...
template <> Size FiniteVolumeEquation<Vector2D>::getRank() const {
  return 2 * field_.grid()->localCells().size();
}

//- Functions
template <>
FiniteVolumeEquation<Vector2D> operator*(const ScalarFiniteVolumeField &lhs,
                                         FiniteVolumeEquation<Vector2D> rhs) {
