...
MotionProfile::MotionProfile(const Point2D &pos,
                             const Vector2D &vel,
                             const Vector2D &acc,
                             Scalar theta,
                             Scalar omega,
                             Scalar alpha)
    :
      Motion(pos, vel, acc, theta, omega, alpha),
      time_(0.)
{
...
{
    auto it = std::upper_bound(timePoints_.begin(), timePoints_.end(), time_, [](Scalar time, const TimePoint &tp)
    { return time < tp.time; });
...
void MotionProfile::addMotion(Scalar startTime, const std::shared_ptr<Motion> &motion)
{
