...
std::pair<Point2D, bool> intersection(const Line2D &lnA, const Line2D &lnB) {
  auto A = StaticMatrix<2, 2>({lnA.d().x, -lnB.d().x, lnA.d().y, -lnB.d().y});

  auto b =
      StaticMatrix<2, 1>({-lnA.r0().x + lnB.r0().x, -lnA.r0().y + lnB.r0().y});
...
std::pair<Point2D, bool> intersection(const Ray2D &ray,
  ...
  auto A = StaticMatrix<2, 2>({ray.r().x, -(ln.ptB().x - ln.ptA().x), ray.r().y,
                               -(ln.ptB().y - ln.ptA().y)});

  auto b =
      StaticMatrix<2, 1>({-ray.x0().x + ln.ptA().x, -ray.x0().y + ln.ptA().y});
