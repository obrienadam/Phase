...
Polygon::Polygon(
    const boost::geometry::model::ring<Point2D, false, true> &boostRing)
    : poly_(boostRing) {
  init();
}

Scalar Polygon::perimeter() const { return boost::geometry::perimeter(poly_); }
...
bool Polygon::isValid() const { return boost::geometry::is_valid(poly_); }

bool Polygon::isEmpty() const { return vertices().size() == 0; }
...
std::vector<Polygon> intersection(const Polygon &pgnA, const Polygon &pgnB) {
  ...
  boost::geometry::intersection(pgnA.boostRing(), pgnB.boostRing(),
                                boostPolygons);
  std::vector<Polygon> polygons(boostPolygons.size());

  std::transform(
      boostPolygons.begin(), boostPolygons.end(), polygons.begin(),
...
std::vector<Polygon> difference(const Polygon &pgnA, const Polygon &pgnB) {
  std::vector<boost::geometry::model::ring<Point2D, false, true>> boostPolygons;
  boost::geometry::difference(pgnA.boostRing(), pgnB.boostRing(),
                              boostPolygons);
  std::vector<Polygon> polygons(boostPolygons.size());
