...
const Scalar Vector2D::EPSILON_ =
    10. *
    std::numeric_limits<Scalar>::epsilon(); // This parameter governs the
                                            // precision of vector operations

Vector2D::Vector2D(std::string vecStr) {
  using namespace std;
  using namespace boost::algorithm;

  vecStr = vecStr.substr(vecStr.find_first_of("(") + 1,
                         vecStr.find_last_of(")") - 1);
...
Vector2D Vector2D::unitVec() const { return *this / mag(); }
...
Scalar Vector2D::angle() const { return std::atan2(y, x); }
...
Vector2D operator-(const Vector2D &rhs) { return Vector2D(-rhs.x, -rhs.y); }
...
Vector2D operator*(Scalar lhs, Vector2D rhs) { return rhs * lhs; }
