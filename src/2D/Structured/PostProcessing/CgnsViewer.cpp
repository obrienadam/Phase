...
CgnsViewer::CgnsViewer(const Input &input, const Solver &solver)
    :
      _solver(solver)
{
    boost::filesystem::path path = "solution/Proc" + std::to_string(_solver.grid()->comm().rank());
    ...
    int zid = file.createStructuredZone(bid,
                                        "Zone",
                                        _solver.grid()->nNodesI(),
                                        _solver.grid()->nNodesJ(),
                                        _solver.grid()->nCellsI(),
                                        _solver.grid()->nCellsJ());
    ...
    std::transform(_solver.grid()->cells().begin(), _solver.grid()->cells().end(), ids.begin(), [](const Cell &c)
    { return c.gid(); });
    ...
    std::string scalarFields = input.caseInput().get<std::string>("Viewer.scalarFields", "");
    std::string vectorFields = input.caseInput().get<std::string>("Viewer.vectorFields", "");

    //split(integerFields_, integerFields, is_any_of(", "), token_compress_on);
    boost::algorithm::split(_scalarFields, scalarFields, boost::algorithm::is_any_of(", "), boost::algorithm::token_compress_on);
    boost::algorithm::split(_vectorFields, vectorFields, boost::algorithm::is_any_of(", "), boost::algorithm::token_compress_on);
}
