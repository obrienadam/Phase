...
public:
  ...
  virtual Scalar getStartTime() const override { return 0.; }
  ...
  virtual void setInitialConditions(const CommandLine &cl,
                                    const Input &input) override;

  //- Timestep
  virtual Scalar maxTimeStep() const { return _maxTimeStep; }
  ...
  virtual const Communicator &comm() const override { return _grid->comm(); }

  std::shared_ptr<StructuredGrid2D> &grid() { return _grid; }

  std::shared_ptr<const StructuredGrid2D> grid() const { return _grid; }

  //- Field management

  template <class T> Field<T> &addField(const std::string &name);
...
protected:
  ...
  struct EqualTo {
    bool operator()(const std::shared_ptr<ScalarField> &lhs,
                    const std::shared_ptr<ScalarField> &rhs) const {
      return lhs->name() == rhs->name();
    }

    bool operator()(const std::shared_ptr<VectorField> &lhs,
                    const std::shared_ptr<VectorField> &rhs) const {
      return lhs->name() == rhs->name();
    }
  };
