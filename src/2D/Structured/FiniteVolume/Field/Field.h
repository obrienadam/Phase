...
public:
  Field(const std::string &name,
        ...
        bool cellField = true, bool faceField = true);
  ...
  Field(const std::string &name,
        const std::shared_ptr<const StructuredGrid2D> &grid, const Input &input,
        bool cellField = true, bool faceField = true);
  ...
  const std::shared_ptr<const StructuredGrid2D> &grid() const { return _grid; }

  T &operator()(const Cell &cell) { return _cellData[cell.lid()]; }

  const T &operator()(const Cell &cell) const { return _cellData[cell.lid()]; }

  T &operator()(const Face &face) { return _faceData[face.lid()]; }

  const T &operator()(const Face &face) const { return _faceData[face.lid()]; }
  ...
  //- History
  Field<T> &oldField(int i) { return *_prevFields[i].second; }

  const Field<T> &oldField(int i) const { return *_prevFields[i].second; }
