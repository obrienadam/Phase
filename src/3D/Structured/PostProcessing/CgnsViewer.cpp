...
CgnsViewer::CgnsViewer(const Input &input,
                       const std::weak_ptr<const Solver> &solver)
    : _solver(solver) {
  boost::filesystem::path path =
      "solution/Proc" + std::to_string(_solver.lock()->grid()->comm().rank());
  ...
  int zid = file.createStructuredZone(
      bid, "Zone", _solver.lock()->grid()->nNodesI(),
      _solver.lock()->grid()->nNodesJ(), _solver.lock()->grid()->nNodesK(),
      _solver.lock()->grid()->nCellsI(), _solver.lock()->grid()->nCellsJ(),
...
void CgnsViewer::write(Scalar time) {
  boost::filesystem::path path =
      "solution/" + std::to_string(time) + "/Proc" +
  ...
  //    "/Grid/Zone/Info/GlobalID"); file.linkNode(bid, zid, sid, "ProcNo",
  //    path.c_str(), "/Grid/Zone/Info/ProcNo");
  file.close();
}
