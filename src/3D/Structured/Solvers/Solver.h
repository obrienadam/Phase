...
public:
    ...
    Solver(const Input &input, const std::shared_ptr<const StructuredGrid3D> &grid);
    ...
    template<class T>
    const std::shared_ptr<Field<T>> &addField(const std::string &name, const Input &input);

    //- Get fields

    const std::unordered_map<std::string, std::shared_ptr<ScalarField>> &scalarFields() const
