...
int main(int argc, char *argv[]) {
  ...
  //    cl.addSwitch("restart,r", "restart the solution from the latest time
  //    point"); cl.addSwitch("use-partitioned-grid,g", "use the pre-partitioned
  //    grid");
  ...
  Plane pn(Point3D(3., 0.431, 0.978), Point3D(1., 3., -1.),
           Point3D(1., 1., 0.2345));
  ...
  std::cout << intersection(
                   pn, LineSegment3D(Point3D(-1, 0, -2), Point3D(2, 2, -4)))
                   .first
            << "\n";
  ...
  //    //PostProcessing postProcessing(input, *solver, std::weak_ptr<const
  //    ImmersedBoundary>());
  ...
  //    auto solver = SolverFactory().create(SolverFactory::POISSON, input,
  //    grid);
