...
public:
  ...
  const Rectangle &shape() const { return _shape; }

  Index idx() const { return _idx; }
  ...
  const Vector3D &norm() const { return _shape.norm(); }

  bool isBoundaryFace() const { return _isBoundaryFace; }
