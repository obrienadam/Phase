...
public:
  ...
  StructuredGrid3D(Size nCellsI = 0, Size nCellsJ = 1, Size nCellsK = 1,
                   Scalar lx = 1., Scalar ly = 1., Scalar lz = 1.);

  StructuredGrid3D(const Input &input);

  //- Grid info
  Scalar lx() const { return _lx; }

  Scalar ly() const { return _ly; }

  Scalar lz() const { return _lz; }

  Size nNodesI() const { return _nCellsI + 1; }

  Size nNodesJ() const { return _nCellsJ + 1; }

  Size nNodesK() const { return _nCellsK + 1; }

  Size nCellsI() const { return _nCellsI; }

  Size nCellsJ() const { return _nCellsJ; }

  Size nCellsK() const { return _nCellsK; }

  Size nCells() const { return _nCells; }

  Size iMax() const { return _nCellsI - 1; }

  Size jMax() const { return _nCellsJ - 1; }

  Size kMax() const { return _nCellsK - 1; }
  ...
  const std::vector<Point3D> &nodes() const { return _nodes; }
  ...
  const std::vector<Face> &kfaces() const { return _kfaces; }
