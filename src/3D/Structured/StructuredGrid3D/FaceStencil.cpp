...
FaceStencil::FaceStencil(const Cell &cell, Face::Direction dir, int forwardBias,
                         int backwardBias)
    : _cell(cell), _face(cell.grid()(cell, dir)), _dir(dir) {
...
Size FaceStencil::maxForwardShift() {
  switch (_dir) {
  case Face::I_POS:
    return _cell.grid().iMax() - _cell.i();
  case Face::I_NEG:
    return _cell.i();
  case Face::J_POS:
    return _cell.grid().jMax() - _cell.j();
  case Face::J_NEG:
    return _cell.j();
  case Face::K_POS:
    return _cell.grid().kMax() - _cell.k();
  case Face::K_NEG:
    return _cell.k();
  }
}
...
const Cell &FaceStencil::operator()(Size shift) const {
  ...
  switch (_dir) {
  case Face::I_POS:
    return _cell.grid()(i + shift, j, k);
  case Face::I_NEG:
    return _cell.grid()(i - shift, j, k);
  case Face::J_POS:
    return _cell.grid()(i, j + shift, k);
  case Face::J_NEG:
    return _cell.grid()(i, j - shift, k);
  case Face::K_POS:
    return _cell.grid()(i, j, k + shift);
  case Face::K_NEG:
    return _cell.grid()(i, j, k - shift);
  }
}
