...
public:
    ...
    //- Stencils
    const std::vector<GhostCellStencil>& stencils() const { return stencils_; }
    ...
    const CellGroup& freshlyClearedCells() const {  return *freshlyClearedCells_; }
    ...
    //- Motion info
    virtual Vector2D velocity(const Point2D& point) const { return Vector2D(0., 0.); }
    virtual Vector2D trajectory() const { return Vector2D(0., 0.); }
    virtual Scalar angularVelocity() const { return 0.; }
