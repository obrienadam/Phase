                             ...
                             const ImmersedBoundary &ib)
{
    ...
    for (const Cell &cell: fluid)
    {
        for (const InteriorLink &nb: cell.neighbours())
        {
            ...
            if (ibObj && flux < 0.)
            {
                const Cell &stCell = fluid.nearestItem(2. * cell.centroid() - nb.cell().centroid());
                LineSegment2D ln = ibObj->intersectionLine(LineSegment2D(cell.centroid(), nb.cell().centroid()));
                Vector2D eta = nb.rCellVec().unitVec();

                if(stCell.id() == cell.id())
                {
                    Scalar eta1 = dot(cell.centroid(), eta);
                    Scalar eta2 = dot(ln.ptB(), eta);
                    Scalar eta3 = dot(nb.cell().centroid(), eta);

                    Matrix c = Matrix(1, 2, {eta3, 1}) * inverse(Matrix(2, 2, {
                            eta1, 1.,
                            eta2, 1.
                    }));

                    eqn.add(cell, cell, c(0, 0) * std::min(flux, 0.));
                    eqn.addSource(cell, c(0, 1) * std::min(flux, 0.));
                }
                else
                {
        ...
        for (const InteriorLink &nb: cell.neighbours())
        {
            ...
            if (ibObj)
            {
                ...
                if(stCell.id() == cell.id())
                {
                    ...
                    eqn.add(cell, cell, c(0, 0) * flux);
                    eqn.addSource(cell, c(0, 1) * flux);
                }
                else
                {
                    Scalar eta1 = dot(stCell.centroid(), eta);
...
void qibm::computeFaceVelocities(VectorFiniteVolumeField &u, const ImmersedBoundary &ib)
{
    ...
    auto ibInterpolate = [&u, &fluid](const Cell &cell2, const Cell &cell3, const ImmersedBoundaryObject &ibObj) {
        const Cell &cell1 = fluid.nearestItem(2. * cell2.centroid() - cell3.centroid());
    ...
    for (const Face &f: u.grid().interiorFaces())
    {
        ...
        if (!lCellIbObj == !rCellIbObj)
        {
            ...
            u(f) = rCellIbObj ? ibInterpolate(f.lCell(), f.rCell(), *rCellIbObj) : ibInterpolate(f.rCell(), f.lCell(),
                                                                                                 *lCellIbObj);
    }
