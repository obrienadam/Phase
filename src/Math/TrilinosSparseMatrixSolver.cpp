...
TrilinosSparseMatrixSolver::TrilinosSparseMatrixSolver(
    const Communicator &comm, Tpetra::ProfileType pftype)
    : comm_(comm), pftype_(pftype) {
...
TrilinosSparseMatrixSolver::TrilinosSparseMatrixSolver(
    ...
    : comm_(comm), pftype_(mat->getProfileType()), mat_(mat) {
...
void TrilinosSparseMatrixSolver::setRank(int rank) { setRank(rank, rank); }

void TrilinosSparseMatrixSolver::setRank(int rowRank, int colRank) {
  using namespace Teuchos;

  auto rangeMap = rcp(new TpetraMap(
      OrdinalTraits<Tpetra::global_size_t>::invalid(), rowRank, 0, Tcomm_));
  auto domainMap = rcp(new TpetraMap(
      OrdinalTraits<Tpetra::global_size_t>::invalid(), colRank, 0, Tcomm_));

  if (mat_.is_null() || !mat_->getRangeMap()->isSameAs(*rangeMap) ||
      !mat_->getDomainMap()->isSameAs(*domainMap)) {
    rangeMap_ = rangeMap;
  ...
  mat_ = rcp(new TpetraCrsMatrix(rangeMap, 20, pftype_));
}
...
void TrilinosSparseMatrixSolver::set(const CoefficientList &eqn) {
  ...
  for (Index localRow = 0, nLocalRows = eqn.size(); localRow < nLocalRows;
       ++localRow) {
    cols.clear();
    vals.clear();
    ...
    mat_->insertGlobalValues(localRow + minGlobalIndex, cols.size(),
                             vals.data(), cols.data());
  }
...
void TrilinosSparseMatrixSolver::set(const std::vector<Index> &rowPtr,
  ...
  for (Index localRow = 0; localRow < rowPtr.size() - 1; ++localRow) {
    ...
    Size n =
        rend - std::find_if_not(rbeg, rend, [](Index idx) { return idx < 0; });

    mat_->insertGlobalValues(localRow + minGlobalIndex, n, vals.data() + ibegin,
                             colInds.data() + ibegin);
  }
...
std::shared_ptr<TrilinosSparseMatrixSolver>
multiply(const TrilinosSparseMatrixSolver &A,
         const TrilinosSparseMatrixSolver &B, bool transA, bool transB) {}
