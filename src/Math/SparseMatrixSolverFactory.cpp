...
std::shared_ptr<SparseMatrixSolver>
SparseMatrixSolverFactory::create(Type type, const Communicator &comm) const {
  switch (type) {
  case EIGEN:
...
std::shared_ptr<SparseMatrixSolver>
SparseMatrixSolverFactory::create(const std::string &type,
  ...
  if (type == "eigen" || type == "eigen3")
    ...
    throw Exception("SparseMatrixSolverFactory", "create",
                    "bad solver type \"" + type + "\".");
}
