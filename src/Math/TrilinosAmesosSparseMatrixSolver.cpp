...
TrilinosAmesosSparseMatrixSolver::TrilinosAmesosSparseMatrixSolver(
...
Scalar TrilinosAmesosSparseMatrixSolver::solve() {
  if (solver_.is_null()) {
    solver_ = Amesos2::create<TpetraCrsMatrix, TpetraMultiVector>(solverName_,
                                                                  mat_, x_, b_);
...
void TrilinosAmesosSparseMatrixSolver::setup(
...
Matrix TrilinosAmesosSparseMatrixSolver::pseudoInverse() const {
  TpetraMultiVector I(mat_->getRowMap(), mat_->getGlobalNumRows(),
                      true); //- m x m identity matrix
  ...
  auto A = Teuchos::rcp(
      new TpetraCrsMatrix(mat_->getDomainMap(), 12)); //- matrix will be n x n
  auto x = Teuchos::rcp(new TpetraMultiVector(mat_->getDomainMap(),
                                              mat_->getGlobalNumRows(),
                                              false)); //- soln will be n x m
  auto b = Teuchos::rcp(new TpetraMultiVector(mat_->getDomainMap(),
                                              mat_->getGlobalNumRows(),
                                              false)); //- rhs will be n x m

  Tpetra::MatrixMatrix::Multiply(*mat_, true, *mat_, false, *A, true);
  mat_->apply(I, *b, Teuchos::TRANS);

  auto solver =
      Amesos2::create<TpetraCrsMatrix, TpetraMultiVector>("klu2", A, x, b);
