...
CooEquation::CooEquation(Size nRows, Size nnz) : rhs_(nRows, 0.), rank_(0) {
...
void CooEquation::addRow(Size nnz) { addRows(1, nnz); }
...
void CooEquation::setCoeff(Index localRow, Index globalCol, Scalar val) {
  ...
  entries_.erase(std::remove_if(entries_.begin(), entries_.end(),
                                [localRow, globalCol](const SparseEntry &e) {
                                  return e.row == localRow &&
                                         e.col == globalCol;
                                }),
                 entries_.end());
