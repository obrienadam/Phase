...
public:
  ...
  //- Special assignment operators ensure the sparse solver isn't accidently
  //destroyed
  Equation &operator=(const Equation &eqn);

  Equation &operator=(Equation &&eqn);

  Size rank() const { return coeffs_.size(); }
  ...
  const SparseMatrixSolver::CoefficientList &coeffs() const { return coeffs_; }
