...
TrilinosBelosSparseMatrixSolver::TrilinosBelosSparseMatrixSolver(
...
void TrilinosBelosSparseMatrixSolver::setRank(int rank) {
  ...
  precon_ = Ifpack2::Factory().create(
      precType_, rcp_static_cast<const TpetraRowMatrix>(mat_));
...
void TrilinosBelosSparseMatrixSolver::setup(
  ...
  if (filename.empty()) {
    belosParams_->set("Maximum Iterations",
                      parameters.get<int>("maxIters", 500));
    belosParams_->set("Convergence Tolerance",
                      parameters.get<Scalar>("tolerance", 1e-8));
  } else
    belosParams_ = Teuchos::getParametersFromXmlFile("case/" + filename);

  solver_ = SolverFactory().create(
      parameters.get<std::string>("solver", "BICGSTAB"), belosParams_);
  ...
  if (filename.empty()) {
    ...
    ifpackParams_->set(
        ...
        parameters.get<std::string>("innerPreconditioner", "RILUK"));
    ifpackParams_->set("schwarz: num iterations",
                       parameters.get<int>("schwarzIters", 1));
    ifpackParams_->set(
        "schwarz: combine mode",
        parameters.get<std::string>("schwarzCombineMode", "ADD"));
    ifpackParams_->set("schwarz: overlap level",
                       parameters.get<int>("schwarzOverlap", 0));
...
void TrilinosBelosSparseMatrixSolver::printStatus(
