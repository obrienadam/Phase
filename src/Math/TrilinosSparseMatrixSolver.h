...
public:
  ...
  Scalar x(Index idx) const { return xData_[idx]; }

  const Communicator &comm() const { return comm_; }

  virtual bool supportsMPI() const { return true; }

  virtual void printStatus(const std::string &msg) const;

  const Teuchos::RCP<TpetraCrsMatrix> &mat() const { return mat_; }
...
multiply(const TrilinosSparseMatrixSolver &A,
         const TrilinosSparseMatrixSolver &B, bool transA = false,
         bool transB = false);
