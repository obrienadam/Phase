...
void CgnsFile::close() { cg_close(_fid); }

int CgnsFile::createBase(const std::string &basename, int cellDim,
                         int physDim) {
  int bid;
...
std::tuple<int, int>
CgnsFile::writeCoordinates(int bid, int zid,
                           const std::vector<Point2D> &coords) {
  std::vector<double> x(coords.size()), y(coords.size());
  std::transform(coords.begin(), coords.end(), x.begin(),
                 [](const Point2D &pt) { return pt.x; });
  std::transform(coords.begin(), coords.end(), y.begin(),
                 [](const Point2D &pt) { return pt.y; });

  std::tuple<int, int> cid;
  cg_coord_write(_fid, bid, zid, CGNS_ENUMV(RealDouble), "CoordinateX",
                 x.data(), &std::get<0>(cid));
  cg_coord_write(_fid, bid, zid, CGNS_ENUMV(RealDouble), "CoordinateY",
                 y.data(), &std::get<1>(cid));
  return cid;
}
...
CgnsFile::writeCoordinates(int bid, int zid,
  ...
  std::transform(coords.begin(), coords.end(), x.begin(),
                 [](const Point3D &pt) { return pt.x; });

  std::transform(coords.begin(), coords.end(), y.begin(),
                 [](const Point3D &pt) { return pt.y; });

  std::transform(coords.begin(), coords.end(), z.begin(),
                 [](const Point3D &pt) { return pt.z; });
  ...
  cg_coord_write(_fid, bid, zid, CGNS_ENUMV(RealDouble), "CoordinateY",
                 y.data(), &std::get<1>(cid));
  cg_coord_write(_fid, bid, zid, CGNS_ENUMV(RealDouble), "CoordinateZ",
                 z.data(), &std::get<2>(cid));
  return cid;
}

template <> std::vector<Point2D> CgnsFile::readCoords(int bid, int zid) const {
  char buff[256];
  cgsize_t size[3];
  ...
  cg_coord_read(_fid, bid, zid, "CoordinateX", CGNS_ENUMV(RealDouble), &rmin,
                &rmax, xCoord.data());
  cg_coord_read(_fid, bid, zid, "CoordinateY", CGNS_ENUMV(RealDouble), &rmin,
                &rmax, yCoord.data());

  std::vector<Point2D> coords(size[0]);
  std::transform(xCoord.begin(), xCoord.end(), yCoord.begin(), coords.begin(),
                 [](Scalar x, Scalar y) { return Point2D(x, y); });

  return coords;
}

int CgnsFile::createStructuredZone(int bid, const std::string &zonename,
                                   int nNodesI, int nNodesJ, int nCellsI,
                                   int nCellsJ) {
  int zid;
  cgsize_t size[] = {nNodesI, nNodesJ, nCellsI, nCellsJ, 0, 0};

  cg_zone_write(_fid, bid, zonename.c_str(), size, CGNS_ENUMV(Structured),
                &zid);
...
int CgnsFile::createStructuredZone(int bid, const std::string &zonename,
  ...
  int zid;
  cgsize_t size[] = {nNodesI, nNodesJ, nNodesK, nCellsI, nCellsJ,
                     nCellsK, 0,       0,       0};
...
int CgnsFile::createUnstructuredZone(int bid, const std::string &zonename,
                                     int nNodes, int nCells) {
  int zid;
  cgsize_t size[] = {nNodes, nCells, 0};
  cg_zone_write(_fid, bid, zonename.c_str(), size, CGNS_ENUMV(Unstructured),
                &zid);
  return zid;
}
...
CgnsFile::Section CgnsFile::readSection(int bid, int zid, int sid) const {
  ...
  cg_section_read(_fid, bid, zid, sid, buff, &type, &section.start,
                  &section.end, &section.nbndry, &section.parentFlag);
  ...
  auto getNVerts = [](CGNS_ENUMT(ElementType_t) type) {
    ...
    case CGNS_ENUMV(QUAD_4):
      return 4;
    default:
      throw Exception("CgnsFile", "readSection",
                      "unsupported element type \"" +
                          std::string(cg_ElementTypeName(type)) + "\".\n");
    }
  };
  ...
  if (type == CGNS_ENUMV(MIXED)) {
    ...
    for (int i = 0; i < section.end - section.start + 1; ++i) {
      eptr.push_back(eptr.back() + nVerts);
      eind.insert(eind.end(), elements.begin() + nVerts * i,
                  elements.begin() + nVerts * (i + 1));
    }
  }
...
int CgnsFile::writeMixedElementSection(int bid, int zid,
  ...
  for (auto i = 0; i < eptr.size() - 1; ++i) {
    ...
    elements.insert(elements.end(), eind.begin() + eptr[i],
                    eind.begin() + eptr[i + 1]);
  }

  int sid;
  cg_section_write(_fid, bid, zid, sectionname.c_str(), CGNS_ENUMV(MIXED),
                   start, end, 0, elements.data(), &sid);

  return sid;
}

int CgnsFile::writeBarElementSection(int bid, int zid,
                                     const std::string &sectionname, int start,
                                     int end,
...
CgnsFile::BoCo CgnsFile::readBoCo(int bid, int zid, int bcid) const {
  ...
  cg_boco_info(_fid, bid, zid, bcid, buff, &type, &ptSetType, &npts,
...
int CgnsFile::writeBoCo(int bid, int zid, const std::string &bcname, int start,
                        int end) {
  int bcid;
  cgsize_t pnts[] = {start, end};
  cg_boco_write(_fid, bid, zid, bcname.c_str(), CGNS_ENUMV(BCGeneral),
                CGNS_ENUMV(PointRange), 2, pnts, &bcid);
  cg_boco_gridlocation_write(_fid, bid, zid, bcid, CGNS_ENUMV(EdgeCenter));
  return bcid;
}

void CgnsFile::linkNode(int bid, int zid, const std::string &nodename,
...
void CgnsFile::linkNode(int bid, int zid, int sid, const std::string &nodename,
                        const std::string &filename,
                        const std::string &nameInFile) {
...
int CgnsFile::writeSolution(int bid, int zid, const std::string &solnname) {
  int solid;
  cg_sol_write(_fid, bid, zid, solnname.c_str(), CGNS_ENUMV(CellCenter),
               &solid);
  return solid;
}
...
template <>
CgnsFile::Field<int> CgnsFile::readField(int bid, int zid, int sid, int rmin,
                                         int rmax,
  ...
  cg_field_read(_fid, bid, zid, sid, field.name.c_str(), CGNS_ENUMV(Integer),
                field.rmin.data(), field.rmax.data(), field.data.data());

  return field;
}

template <>
CgnsFile::Field<double>
CgnsFile::readField(int bid, int zid, int sid, int rmin, int rmax,
                    const std::string &fieldname) const {
  Field<double> field;
  ...
  cg_field_read(_fid, bid, zid, sid, field.name.c_str(), CGNS_ENUMV(RealDouble),
                field.rmin.data(), field.rmax.data(), field.data.data());
...
template <>
int CgnsFile::writeField(int bid, int zid, int sid,
                         const std::string &fieldname,
                         const std::vector<int> &field) {
  int fid;
  cg_field_write(_fid, bid, zid, sid, CGNS_ENUMV(Integer), fieldname.c_str(),
                 field.data(), &fid);
  return fid;
}
...
int CgnsFile::writeField(int bid, int zid, int sid,
                         const std::string &fieldname,
                         const std::vector<Label> &field) {
  return writeField(bid, zid, sid, fieldname,
                    std::vector<int>(field.begin(), field.end()));
}
...
int CgnsFile::writeField(int bid, int zid, int sid,
  ...
  int fid;
  auto data = field;
  cg_field_write(_fid, bid, zid, sid, CGNS_ENUMV(RealDouble), fieldname.c_str(),
                 data.data(), &fid);
  return fid;
}
...
int CgnsFile::writeField(int bid, int zid, int sid,
                         const std::string &fieldname,
                         const std::vector<Vector2D> &field) {
  int fid;
  std::vector<double> xComp(field.size()), yComp(field.size());

  std::transform(field.begin(), field.end(), xComp.begin(),
                 [](const Vector2D &u) { return u.x; });

  std::transform(field.begin(), field.end(), yComp.begin(),
...
void CgnsFile::writeDescriptorNode(int bid, const std::string &name,
                                   const std::string &text) {
  cg_goto(_fid, bid, "end");
  cg_descriptor_write(name.c_str(), text.c_str());
}

int CgnsFile::writeDescriptorNode(int bid, int zid, int sid,
...
CgnsFile::readDescriptorNodes(int bid, int zid, int sid) const {
  using namespace std;
  int ndescriptors;
  char name[256];
