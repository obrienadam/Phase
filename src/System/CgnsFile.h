...
public:
  ...
  std::tuple<int, int> writeCoordinates(int bid, int zid,
                                        const std::vector<Point2D> &coords);

  std::tuple<int, int, int>
  writeCoordinates(int bid, int zid, const std::vector<Point3D> &coords);

  template <class T> std::vector<T> readCoords(int bid, int zid) const;

  int createStructuredZone(int bid, const std::string &zonename, int nNodesI,
                           int nNodesJ, int nCellsI, int nCellsJ);

  int createStructuredZone(int bid, const std::string &zonename, int nNodesI,
                           int nNodesJ, int nNodesK, int nCellsI, int nCellsJ,
                           int nCellsK);

  int createUnstructuredZone(int bid, const std::string &zonename, int nNodes,
                             int nCells);
  ...
  int writeMixedElementSection(int bid, int zid, const std::string &sectionname,
                               int start, int end, const std::vector<int> &eptr,
  ...
  int writeBoCo(int bid, int zid, const std::string &bcname, int start,
                int end);

  //- linking

  void linkNode(int bid, int zid, const std::string &nodename,
                const std::string &filename, const std::string &nameInFile);

  void linkNode(int bid, int zid, int sid, const std::string &nodename,
                const std::string &filename, const std::string &nameInFile);
  ...
  //- Fields
  template <class T>
  Field<T> readField(int bid, int zid, int sid, int rmin, int rmax,
                     const std::string &fieldname) const;

  template <class T>
  int writeField(int bid, int zid, int sid, const std::string &fieldname,
  ...
  void writeDescriptorNode(int bid, const std::string &name,
                           const std::string &text);

  int writeDescriptorNode(int bid, int zid, int sid, const std::string &name,
                          const std::string &text);

  std::vector<std::pair<std::string, std::string>>
  readDescriptorNodes(int bid, int zid, int sid) const;
