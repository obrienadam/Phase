...
void RunControl::run(const CommandLine &cl, const Input &input,
                     SolverInterface &solver,
                     PostProcessingInterface &postProcessing) {
  //- Max run time (for easy restart)
  auto maxWallTime =
      input.caseInput().get<Scalar>("Solver.maxWallTime",
  ...
  Scalar timeStep = input.caseInput().get<Scalar>("Solver.initialTimeStep",
                                                  solver.maxTimeStep());
  ...
  for (size_t iterNo = 0;
       ...
       time += timeStep, timeStep = solver.computeMaxTimeStep(maxCo, timeStep),
              ++iterNo) {
    ...
    solver.printf("%s\n", (std::string(96, '-') + "| End of iteration no " +
                           std::to_string(iterNo + 1))
                              .c_str());
  }
  time_.stop();
